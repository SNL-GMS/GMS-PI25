...
import { closeContextMenu, Form, FormTypes, WidgetTypes } from '@gms/ui-core-components';
...
export function QuiteMenuItem(props: QuietAction) {
  ...
  const userSessionState = useAppSelector(state => state.app.userSession);
  return (
    <MenuItem2 text="Quiet for..." disabled={disabled} data-cy="quiet-without-comment">
...
export const showQuietingContextMenu = (q: QuietAction): void => {
  ...
  showContextMenu({
    content: QuietingContextMenu,
    targetOffset: q.position
  });
};
