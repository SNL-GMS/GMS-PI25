...
describe('Fetch ACEI Historical data', () => {
  ...
  it('getHistoricalAceiData errors', () => {
    ...
    expect(() => getHistoricalAceiData(undefined)).toThrow(undefinedErr);

    const stationNameErr = new Error(
      'Unable to retrieve historical ACEI data due to missing stationName'
    );

    expect(() => getHistoricalAceiData(historicalAceiInput)).toThrow(stationNameErr);
  });
  ...
  it('getHistoricalAceiData returns undefined for bad station name', () => {
    ...
    expect(() => getHistoricalAceiData(historicalAceiInput)).toThrow(stationNotFoundErr);
  });
  ...
  it('getHistoricalSohData errors', () => {
    ...
    expect(() => getHistoricalSohData(undefined)).toThrow(undefinedErr);

    const stationNameErr = new Error(
      'Unable to retrieve historical SOH data due to missing stationName'
    );

    expect(() => getHistoricalSohData(historicalSohInput)).toThrow(stationNameErr);
  });
  ...
  it('getHistoricalSohData returns undefined for bad station name', () => {
    ...
    expect(() => getHistoricalSohData(historicalSohInput)).toThrow(stationNotFoundErr);
  });
});
