...
# started, but NO config has yet been loaded to the system
STATE_STARTED = 'started'
# config is in the process of being loaded from this service to the system
STATE_LOADING = 'loading'
# config has been loaded to the system (at least once)
STATE_LOADED = 'loaded'
...
#       LOADED   > LOADING  (queue reload: reload will be
#                                successful/unsuccessful)
#
#    load complete (note: state should stay LOADING once queued)
#       if required  services not yet available, requeue load in 5 seconds
...
class State():
    ...
    def __init__(self):
        ...
        start_time = session.query(StateKeyValue).filter_by(key=START_TIME
                                                            ).first()
        initial_state = session.query(StateKeyValue).filter_by(key=STATE
                                                               ).first()
