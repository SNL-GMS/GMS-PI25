...
class EarthModelLoaderTest(unittest.TestCase):
    ...
    def test_get_minio_prefix_map(self):
        with patch(
            'gmsdataloader.earthmodels.EarthModelMinioConfig'
        ) as mock_config:
            ...
            self.assertTrue(
                {
                    'phase': 'S',
                    'path': f'{WORKING_DIR}/resources/traveltime/iaspei/S.json'
                } in prefix_map['traveltime/iaspei']
            )

            self.assertTrue(
                {
                    'phase': 'P',
                    'path': f'{WORKING_DIR}/resources/traveltime/ak135/P.json'
                } in prefix_map['traveltime/ak135']
            )
            self.assertTrue(
                {
                    'phase': 'sP',
                    'path':
                    f'{WORKING_DIR}/resources/traveltime/ak135/~sP.json'
                } in prefix_map['traveltime/ak135']
            )
            ...
            mock_client.fput_object.assert_has_calls(
                [
                    call(
                        'testbucket',
                        'traveltime/ak135/P',
                        f'{WORKING_DIR}/resources/traveltime/ak135/P.json'
                    ),
                    call(
                        'testbucket',
                        'traveltime/ak135/sP',
                    ...
                    call(
                        'testbucket',
                        'traveltime/iaspei/S',
                        f'{WORKING_DIR}/resources/traveltime/iaspei/S.json'
                    )
                ],
                any_order=True
            )  # yapf: disable
