...
class AssertionUtilities {
   ...
   * @param message expected exception message to contain this message, not null
   */
  private static <T extends Throwable> void verifyThrows(
      Executable executable, Class<T> throwableClass, String message) {
   ...
   * @param executable {@link Executable} to run, not null
   * @param throwableClass expected throwable type, not null
   * @param messageComponents expected exception message to contain each of these messages, not null
   */
  static <T extends Throwable> void verifyThrowsWithAllInMessage(
      Executable executable, Class<T> throwableClass, Collection<String> messageComponents) {

    final String actualMessage = assertThrows(throwableClass, executable).getMessage();
    assertAll(
        messageComponents.stream()
            .map(
                e ->
                    () ->
                        assertTrue(
                            actualMessage.contains(e),
                            "Expected msg to contain: '"
                                + e
                                + "', but not in message which was: '"
                                + actualMessage
                                + "'")));
  }
}
