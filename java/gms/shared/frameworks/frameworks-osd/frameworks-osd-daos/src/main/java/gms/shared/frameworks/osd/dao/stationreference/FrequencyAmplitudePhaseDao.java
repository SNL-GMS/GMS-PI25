...
public class FrequencyAmplitudePhaseDao {
  ...
  @GeneratedValue(
      ...
      generator = "frequency_amplitude_phase_sequence")
  @SequenceGenerator(
      name = "frequency_amplitude_phase_sequence",
      sequenceName = "frequency_amplitude_phase_sequence",
      allocationSize = 5)
  private long id;

  @Type(DoubleArrayType.class)
  @Column(name = "frequencies", columnDefinition = "double precision[]")
  ...
  @Type(DoubleArrayType.class)
  @Column(name = "amplitude_response", columnDefinition = "double precision[]")
  private double[] amplitudeResponse;

  @Type(DoubleArrayType.class)
  @Column(name = "amplitude_response_standard_deviation", columnDefinition = "double precision[]")
  ...
  @Type(DoubleArrayType.class)
  @Column(name = "phase_response", columnDefinition = "double precision[]")
  private double[] phaseResponse;

  @Type(DoubleArrayType.class)
  @Column(name = "phase_response_standard_deviation", columnDefinition = "double precision[]")
  ...
  public static FrequencyAmplitudePhaseDao from(FrequencyAmplitudePhase fap) {
    Preconditions.checkNotNull(
        fap, "Cannot create FrequencyAmplitudePhaseDao from null FrequencyAmplitudePhase");
  ...
  public FrequencyAmplitudePhase toCoi() {
    ...
    Validate.isTrue(
        this.amplitudeResponseStdDev.length > 0, "amplitudeResponseStdDev length must be > 0");
    Validate.isTrue(this.phaseResponse.length > 0, "phaseResponse length must be > 0");
    Validate.isTrue(
        this.phaseResponseStandardDeviation.length > 0, "phaseResponseStdDev length must be > 0");
    Validate.isTrue(
        this.frequencies.length == this.amplitudeResponse.length
  ...
  public void setAmplitudeResponseUnits(Units amplitudeResponseUnits) {
  ...
  public void setPhaseResponseUnits(Units phaseResponseUnits) {
  ...
  public String toString() {
    return "FrequencyAmplitudePhaseDao{"
        ...
        + amplitudeResponseUnits
        + ", amplitudeResponse="
        + Arrays.toString(amplitudeResponse)
        + ", amplitudeResponseStdDev="
        + Arrays.toString(amplitudeResponseStdDev)
        + ", phaseResponseUnits="
        + phaseResponseUnits
        + ", phaseResponse="
        + Arrays.toString(phaseResponse)
        + ", phaseResponseStandardDeviation="
        + Arrays.toString(phaseResponseStandardDeviation)
        + '}';
  }
}
