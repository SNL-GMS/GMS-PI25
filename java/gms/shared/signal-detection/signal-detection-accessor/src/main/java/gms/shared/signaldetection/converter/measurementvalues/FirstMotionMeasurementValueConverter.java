...
public final class FirstMotionMeasurementValueConverter
    implements MeasurementValueConverter<FirstMotionMeasurementValue> {
  ...
  @Override
  public Optional<FirstMotionMeasurementValue> convert(
    ...
    String featureMeasurementTypeCode =
        spec.getFeatureMeasurementTypeCode()
            .orElseThrow(
                () -> new NoSuchElementException("Feature measurement type index does not exist"));
    return Optional.of(
        FirstMotionMeasurementValue.from(
            FirstMotionType.fromCode(featureMeasurementTypeCode), Optional.empty(), optionalTime));
  }
}
