  ...
  static PredictionsAndBehaviors fromLegacyToPredictionsAndBehaviors(
    ...
    var arrivalTimeBehavior =
        arrivalTimeMeasurement.flatMap(
            measurement -> // if arrivalTimePrediction is present (meaning arInfoDao is also
                // present)
                ...
                rToSAzimuthPrediction
                    .map(
                        prediction -> {
                          var weight =
                              DoubleMath.fuzzyEquals(
                                          arInfoDaoOpt.get().getAzimuthImport(), -1.0, NA_EPSILON)
                                      || !isDefining(assocDao.getAzimuthDefining())
                                  ? Optional.<Double>empty()
                ...
                slownessPrediction
                    .map(
                        prediction -> {
                          var weight =
                              DoubleMath.fuzzyEquals(
                                          arInfoDaoOpt.get().getSlownessImport(), -1.0, NA_EPSILON)
                                      || !isDefining(assocDao.getSlownessDefining())
                                  ? Optional.<Double>empty()
