...
public class SystemConfig {
  ...
  private SystemConfig(String componentName, List<SystemConfigRepository> repositories) {
    ...
    try {
      ...
      // This occurs if there is a value defined in SystemConfig that has not been defined in the
      // log4j confg file
      // in this case we just use whatever is defined log4j config file
      if (logLevel == null) {
        LOGGER.warn(
            "The specified LogLevel has not been defined in the log4j.xml configuration file.  "
      ...
      LOGGER.info(
          "No log_level defined in SystemConfig.  Defaulting to the level defined in log4j.xml"
              + " file.");
    }
  }
