...
public class ReferenceChannelDao {
  ...
  public ReferenceChannelDao(ReferenceChannel channel) {
    ...
    this.aliases =
        channel.getAliases().stream().map(ReferenceAliasDao::new).collect(Collectors.toList());
  }
  ...
  public ReferenceChannel toCoi() {
    List<ReferenceAlias> aliasList =
        this.aliases.stream().map(ReferenceAliasDao::toCoi).collect(Collectors.toList());
