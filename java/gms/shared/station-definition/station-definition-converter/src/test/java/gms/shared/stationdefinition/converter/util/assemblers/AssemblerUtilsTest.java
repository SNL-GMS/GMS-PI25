...
class AssemblerUtilsTest {
  ...
  void testBuildVersionTable() {
    ...
    siteChanKeyTimeMultimap.putAll(
        siteChanKey1,
        List.of(
            Instant.parse("2006-12-30T13:35:24.00Z"), Instant.parse("2009-06-13T12:35:24.00Z")));

    siteChanKeyTimeMultimap.putAll(
        siteChanKey2,
        List.of(
            Instant.parse("2010-12-30T13:35:24.00Z"),
            Instant.parse("2011-06-13T12:35:24.00Z"),
            Instant.parse("2011-06-13T12:35:24.00Z")));
    List<SensorDao> sensorDaos =
        UtilsTestFixtures.getListOfSensorsFromMultimap(siteChanKeyTimeMultimap);

    NavigableMap<Instant, SensorDao> navigableMap1 =
        sensorDaos.stream()
            .filter(sensorDao -> "STA1".equals(sensorDao.getSensorKey().getStation()))
            .collect(
                Collectors.toMap(
                    Functions.compose(SensorKey::getTime, SensorDao::getSensorKey),
                    Functions.identity(),
                    (val1, val2) ->
                        val1.getSensorKey().getTime().isAfter(val2.getSensorKey().getTime())
                            ? val1
                            : val2,
                    TreeMap::new));
    ...
    NavigableMap<Instant, SensorDao> navigableMap2 =
        sensorDaos.stream()
            .filter(sensorDao -> "STA2".equals(sensorDao.getSensorKey().getStation()))
            .collect(
                Collectors.toMap(
                    Functions.compose(SensorKey::getTime, SensorDao::getSensorKey),
                    Functions.identity(),
  ...
  @Test
  void testEffectiveAtStartOfDayOffset() {
    Instant someTime =
        AssemblerUtils.effectiveAtStartOfDayOffset.apply(Instant.parse("2000-08-13T12:34:56Z"));
  ...
  @Test
  void testEffectiveUntilNoonOffset() {
    Instant someTime =
        AssemblerUtils.effectiveUntilNoonOffset.apply(Instant.parse("2000-08-13T12:34:56Z"));
  ...
  @Test
  void testEffectiveAtNoonOffset() {
    Instant someTime =
        AssemblerUtils.effectiveAtNoonOffset.apply(Instant.parse("2000-08-13T12:34:56Z"));
