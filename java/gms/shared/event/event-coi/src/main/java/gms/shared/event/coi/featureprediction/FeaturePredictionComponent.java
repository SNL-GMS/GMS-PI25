 ...
 * sum of its {@link FeaturePredictionComponentType}s which may represent baseline values or
 * corrections to the baseline values.
 *
 * @param <T> the valueType of the FeaturePredictionComponent
 */
@AutoValue
public abstract class FeaturePredictionComponent<T> {
  ...
  public static <T> FeaturePredictionComponent<T> from(
      ...
      @JsonProperty("featurePredictionComponent")
          FeaturePredictionComponentType featurePredictionComponent) {
    return new AutoValue_FeaturePredictionComponent<>(
        value, extrapolated, featurePredictionComponent);
  }
}
