...
import type { Columns, StationStatisticsColumnDefinition } from './column-definitions';
...
export class StationStatisticsTable extends React.Component<StationStatisticsTableProps> {
   ...
   */
  private readonly onCellContextMenu = (event: CellClickedEvent): void => {
    const stationNames = this.getSelectedIdsOnRightClick(event);
    const mouseEvent: MouseEvent = event.event as MouseEvent;
    const offset: Offset = { left: mouseEvent.x, top: mouseEvent.y };
    showContextMenu({
      content: this.props.acknowledgeContextMenu(stationNames),
      targetOffset: offset
    });
   ...
   */
  private readonly getSelectedIdsOnRightClick = (event: CellClickedEvent): string[] => {
