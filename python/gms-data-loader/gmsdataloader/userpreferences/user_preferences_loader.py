...
from gmsdataloader.userpreferences.user_preferences_loader_config import (
...
class UserPreferencesLoader:
    """
    Loads User Preferences Information from the filesystem and publishes
    them to a specified location
    """

    def __init__(
        self,
        config: UserPreferencesLoaderConfig,
    ...
    def load_user_preferences(self, user_preferences_path: str) -> None:
        """
        Loads data for user preferences objects given file paths and publishes
        to the respective configured endpoint
        ...
        with ThreadPoolExecutor(max_workers=1) as executor:
            ...
            future_list.append(
                executor.submit(
                    ...
                    user_prefs_obj_urls.user_preferences_url,
                    self.total_retries
                )
            )
    ...
    def load(self, user_prefs_obj_paths: UserPreferencesObjectPaths) -> None:
        """
        High-level loader function that reads user preference information from
        the filesystem and publishes them to its respective endpoint

        Keyword arguments:
            user_preferences_path -- Path to User Preferences Array (JSON)
        """
        self.load_user_preferences(
            user_preferences_path=user_prefs_obj_paths.user_preferences_path
        )
