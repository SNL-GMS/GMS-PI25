...
public final class NumericFeaturePredictionValue
    extends FeaturePredictionValue<NumericMeasurementValue, DoubleValue, DoubleValue> {
  ...
  public static NumericFeaturePredictionValue from(
      @JsonProperty("featureMeasurementType")
          FeatureMeasurementType<NumericMeasurementValue> featureMeasurementType,
      @JsonProperty("predictedValue") NumericMeasurementValue predictedValue,
      @JsonProperty("derivativeMap")
          Map<FeaturePredictionDerivativeType, DoubleValue> derivativeMap,
      @JsonProperty("featurePredictionComponents")
          Set<FeaturePredictionComponent<DoubleValue>> featurePredictionComponents) {

    var featureMeasurementTypeName = featureMeasurementType.getFeatureMeasurementTypeName();
    var allowedTypeNames =
        Set.of(
            FeatureMeasurementTypes.SLOWNESS.getFeatureMeasurementTypeName(),
            FeatureMeasurementTypes.EMERGENCE_ANGLE.getFeatureMeasurementTypeName(),
            FeatureMeasurementTypes.RECEIVER_TO_SOURCE_AZIMUTH.getFeatureMeasurementTypeName(),
            FeatureMeasurementTypes.SOURCE_TO_RECEIVER_AZIMUTH.getFeatureMeasurementTypeName(),
            FeatureMeasurementTypes.SOURCE_TO_RECEIVER_DISTANCE.getFeatureMeasurementTypeName());

    if (!allowedTypeNames.contains(featureMeasurementTypeName)) {
      throw new IllegalArgumentException(
          "NumericFeaturePredictionValue can only have an SLOWNESS, EMERGENCE_ANGLE,"
              + " RECEIVER_TO_SOURCE_AZIMUTH, SOURCE_TO_RECEIVER_AZIMUTH, or"
              + " SOURCE_TO_RECEIVER_DISTANCE FeatureMeasurementTypes");
    }

    return new NumericFeaturePredictionValue(
        predictedValue, derivativeMap, featurePredictionComponents, featureMeasurementType);
  }
}
