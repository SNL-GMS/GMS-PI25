...
class Sun3FormatWaveformReaderTest {
  private final WaveformReader reader = new Sun3FormatWaveformReader();
  ...
  void testReadTestData() throws Exception {
    try (InputStream is = this.getClass().getResourceAsStream(WFS3_FILE)) {
      WaveformReaderTestUtil.testReadTestData(
          reader, is, SAMPLES_TO_READ, SAMPLES_TO_SKIP, REF_SAMPLES);
    }
  }
  ...
  void testReadTestDataSkipError() throws Exception {
    assertThrows(
        ...
        () ->
            WaveformReaderTestUtil.testReadTestData(
                reader,
                this.getClass().getResourceAsStream(WFS3_FILE),
                SAMPLES_TO_READ,
                -20,
                new double[] {}));
  }
