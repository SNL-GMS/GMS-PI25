...
class DziewonskiGilbertLoaderTest(unittest.TestCase):
    ...
    def test_load(self):
        with patch('minio.Minio') as client_mocker:
            ...
            config = DziewonskiGilbertMinioConfig(
                'http://non-existent-minio-service',
                'testbucket',
                minio_client=mock_client
            )
            loader = DziewonskiGilbertLoader(
                f'{WORKING_DIR}/resources/dziewonski-gilbert',
                config
            )
            ...
            mock_client.put_object.assert_has_calls(
                [
                    call(
                        'testbucket',
                        'dziewonski-gilbert/P',
                        mock.ANY,
                        mock.ANY
                    ),
                    call(
                        'testbucket',
                        'dziewonski-gilbert/PP',
                        mock.ANY,
                        mock.ANY
                    ),
                    call(
                        'testbucket',
                        'dziewonski-gilbert/sP',
                        mock.ANY,
                        mock.ANY
                    ),
                    call(
                        'testbucket',
                        'dziewonski-gilbert/SKKPab',
                        mock.ANY,
                        mock.ANY
                    ),
                ],
