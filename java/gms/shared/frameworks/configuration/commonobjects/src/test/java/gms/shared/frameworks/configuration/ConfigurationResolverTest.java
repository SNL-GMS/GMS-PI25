...
class ConfigurationResolverTest {
  ...
  void setUp() {
    ...
    configOptDefault =
        ConfigurationOption.from(
            "GLOBAL-DEFAULT",
            List.of(DefaultConstraint.from(), snrIsWildcard, bazIsWildcard),
            fooParamsDefaultsFieldMap);
    final ConfigurationOption configOptSnrIs5Default =
        ConfigurationOption.from(
            "GLOBAL-DEFAULT",
            List.of(DefaultConstraint.from(), snrIs5, bazIsWildcard),
            FieldMapUtilities.toFieldMap(fooParamsSnr5Defaults));

    final ConfigurationOption configOptSnrIs5 =
        ConfigurationOption.from("SNR-5", List.of(snrIs5), Map.of("a", 10));
    configOptSnrIs5BazIs10 =
        ConfigurationOption.from("(SNR-5)/(BAZ-10)", List.of(snrIs5, bazIs10), Map.of("a", 20));
    configOptSnrIs5BazIsWildcard =
        ConfigurationOption.from(
            "(SNR-5)/(BAZ-WILDCARD)", List.of(snrIs5, bazIsWildcard), Map.of("a", 30));

    snrIs5ParametersFieldMap =
        FieldMapUtilities.toFieldMap(fooParamsDefaults.toBuilder().a(10).build());
    snrIs5WIthSnr5DefaultParametersFieldMap =
        FieldMapUtilities.toFieldMap(fooParamsSnr5Defaults.toBuilder().a(10).build());
    fooParamsSnrIs5BazIs10FieldMap =
        FieldMapUtilities.toFieldMap(fooParamsDefaults.toBuilder().a(20).build());
    fooParamsSnrIs5BazIsWildcardFieldMap =
        FieldMapUtilities.toFieldMap(fooParamsDefaults.toBuilder().a(30).build());

    configurationSnrIs5 =
        Configuration.from("TEST-CONFIG", List.of(configOptDefault, configOptSnrIs5));
    configurationSnrIs5WithSnr5Default =
        Configuration.from(
            "TEST-CONFIG", List.of(configOptDefault, configOptSnrIs5Default, configOptSnrIs5));
    configurationSnrIs5BazIs10 =
        Configuration.from("TEST-CONFIG", List.of(configOptDefault, configOptSnrIs5BazIs10));
    configurationSnrIs5BazIsWildcard =
        Configuration.from("TEST-CONFIG", List.of(configOptDefault, configOptSnrIs5BazIsWildcard));
  ...
  void testResolvesConfigurationDefaultWhenNoSelectors() {
    assertEquals(
        fooParamsDefaultsFieldMap, ConfigurationResolver.resolve(configurationSnrIs5, List.of()));
  }
  ...
  void testResolvesAmongMultipleDefaults() {
    assertEquals(
        ...
        ConfigurationResolver.resolve(
            configurationSnrIs5WithSnr5Default, List.of(Selector.from("snr", 5.0))));
  }
  ...
  void testResolvesConfigurationOptionWhenSelectorMatchesSingleConstraint() {
    assertEquals(
        snrIs5ParametersFieldMap,
        ConfigurationResolver.resolve(configurationSnrIs5, List.of(Selector.from("snr", 5.0))));
  }
  ...
  void testResolveIgnoresSelectorsNotMatchingAnyConstraintCriterions() {
    assertEquals(
        ...
        ConfigurationResolver.resolve(
            configurationSnrIs5BazIs10,
            List.of(
                Selector.from("bar", -20.0),
                Selector.from("baz", 10.0),
  ...
  void testResolveNoSelectorWithCriterionForWildcardConstraintExpectConfigurationOptionMatch() {
    assertEquals(
        fooParamsSnrIs5BazIsWildcardFieldMap,
        ConfigurationResolver.resolve(
            configurationSnrIs5BazIsWildcard, List.of(Selector.from("snr", 5.0))));
  }
  ...
  void testResolveNoSelectorWithCriterionForNonWildcardConstraintExpectDefaultParameters() {
    assertEquals(
        fooParamsDefaultsFieldMap,
        ConfigurationResolver.resolve(
            configurationSnrIs5BazIsWildcard, List.of(Selector.from("baz", 5.0))));
  }

  /**
   * Ties occur when the Selectors match multiple ConfigurationOptions. The first tie breaker is the
   * number of exact matches (e.g. non-WildcardConstraint matches). In this example the Selectors
   * match both ConfigurationOptions but one of them has 1 exact match and one wildcard while the
   * other has 2 exact matches.
   */
  @Test
  void testResolveUsesNumberOfExactMatchesWhenSelectorsMatchMultipleConfigurationOptions() {
    ...
    assertEquals(
        fooParamsSnrIs5BazIs10FieldMap,
        ConfigurationResolver.resolve(
            configuration, List.of(Selector.from("baz", 10.0), Selector.from("snr", 5.0))));
  }

  /**
   * Ties occur when the Selectors match multiple ConfigurationOptions. The first tie breaker is the
   * number of exact matches (e.g. non-WildcardConstraint matches). The second tie breaker is the
   * priority of the exact matching Constraints. In this example the Selectors match both
   * ConfigurationOptions with 1 exact match and 1 wildcard match.
   */
  @Test
  void testResolveUsesPriorityToBreakTies() {
    final ConfigurationOption configOptSnrIsWildcardBazIs10 =
        ConfigurationOption.from(
            "(SNR-WILDCARD)/(BAZ-10)", List.of(snrIsWildcard, bazIs10), Map.of("a", 40));

    final Map<String, Object> fooParamsSnrIsWildcardBazIs10 =
        FieldMapUtilities.toFieldMap(fooParamsDefaults.toBuilder().a(40).build());
  ...
  void testResolveWorksWhenNumericScalarSelectorIsInt() {
    ...
    assertEquals(
        fooParamsSnrIsWildcardBazIs10,
        ConfigurationResolver.resolve(
            configuration, List.of(Selector.from("baz", 10), Selector.from("snr", 5.0))));
  }
  ...
  void testResolveHasTieExpectIllegalStateException() {
    ...
    final Constraint barIs20 =
        NumericScalarConstraint.from(
            "bar", Operator.from(Type.EQ, false), 20.0, snrIs5.getPriority());
    final Constraint barIsWildcard = WildcardConstraint.from("bar");

    final ConfigurationOption configOptSnrIsWildcardBarIs20 =
        ConfigurationOption.from(
            "(SNR-WILDCARD)/(BAR-20)", List.of(snrIsWildcard, barIs20), Map.of("a", 40));

    final ConfigurationOption configOptSnrIs5BarIsWildcard =
        ConfigurationOption.from(
            "(SNR-5)/(BAR-WILDCARD)", List.of(snrIs5, barIsWildcard), Map.of("a", 30));
    ...
    TestUtilities.expectExceptionAndMessage(
        () ->
            ConfigurationResolver.resolve(
                configuration, List.of(Selector.from("snr", 5.0), Selector.from("bar", 20.0))),
  ...
  void testResolveDuplicateSelectorCriteriaExpectIllegalArgumentException() {
    TestUtilities.expectExceptionAndMessage(
        () ->
            ConfigurationResolver.resolve(
                configurationSnrIs5, List.of(Selector.from("A", 1.0), Selector.from("A", 1.0))),
  ...
  void testResolveFindsNoMatchesExpectIllegalArgumentException() {
    TestUtilities.expectExceptionAndMessage(
        () ->
            ConfigurationResolver.resolve(
                configurationSnrIs5NoDefaults, List.of(Selector.from("snr", -5.0))),
        IllegalStateException.class,
