 ...
 */
@Component
@Qualifier("interval") public class BridgedDataSourceIntervalSimulator implements DataSimulator {
  ...
  private static final Logger logger =
      LoggerFactory.getLogger(BridgedDataSourceIntervalSimulator.class);
   ...
   *
   * @param bridgedDataSourceSimulatorSpec - The {@link DataSimulatorSpec} that specifies simulation
   *     parameters.
   */
  @Override
  public void initialize(DataSimulatorSpec bridgedDataSourceSimulatorSpec) {

    initialize(bridgedDataSourceSimulatorSpec, Instant.now());
  }

  /** Start the interval simulator. */
  @Override
  public void start() {
    simulationFluxDisposable =
        getSimulationFlux(
            ...
            .subscribe(
                shiftedIntervalDao -> {
                  ...
                  logger.debug(
                      "Stored IntervalDao {}", shiftedIntervalDao.getClassEndTimeNameTimeKey());
                });
  }
  ...
  /** Delete all simulated intervals. */
  @Override
  public void cleanup() {
  ...
  //
  /**
   * The process of loading seed data into the simulation set has two parts: BackwardPass, before
   * the simulationStartTime and ForwardPass, after the simulationStartTime. This is the backward
   * pass:
   *
   * <p>BackwardPass - Align the seed data so that it's endTime aligns with the simulationStartTime
   ...
   */
  private long shiftBackward(SeedMetadata seedMetadata, Instant simulationStartTime) {

    long initialBackwardsTimeShift =
        Duration.between(seedMetadata.getAdjustedSeedDataEndTime(), simulationStartTime)
            .toSeconds();

    var maxIntervalIdReference = new AtomicLong();

    logger.info(
        "Copying {} seeds of length {} starting at {} going backwards in time.",
        seedMetadata.getNumberBackwardSeeds(),
    ...
    Flux.range(1, seedMetadata.getNumberBackwardSeeds())
        .map(
            shiftIndex -> {
              ...
              var shiftedList =
                  backwardPassShift(
                      initialBackwardsTimeShift,
                      shiftIndex,
                      seedMetadata.getAdjustedSeedDuration());

              // grab from the shifted list the last id, created for start(), only for non-empty
              // lists
              if (!shiftedList.isEmpty()) {
                var lastId = shiftedList.get(shiftedList.size() - 1).getIntervalIdentifier();
  ...
  /**
   * The process of loading seed data into the simulation set has two parts: BackwardPass, before
   * the simulationStartTime and ForwardPass, after the simulationStartTime. This is the forward
   * pass:
   *
   * <p>ForwardPass - Align the seed data so that it's startTime aligns with simulationStartTime and
   * then copy numberForwardSeeds shifting each by the seed duration
   *
   * @param seedMetadata SeedMetadata object containing metadata need to perform the backward pass
   * @param simulationStartTime Time that the simulation starts.
  ...
  private long shiftForward(
    ...
    long forwardTimeShift =
        Duration.between(seedMetadata.getAdjustedSeedDataStartTime(), simulationStartTime)
            .toSeconds();

    logger.info(
        "Copying {} seeds of length {} starting at {} going forwards in time.",
        seedMetadata.getNumberForwardSeeds(),
        seedMetadata.getAdjustedSeedDuration(),
        simulationStartTime);
  ...
  @VisibleForTesting
  void initialize(DataSimulatorSpec bridgedDataSourceSimulatorSpec, Instant now) {

    var seedMetadata = computeSeedMetadata(bridgedDataSourceSimulatorSpec, now);

    this.initializeSeedData(
        seedMetadata.getAdjustedSeedDataStartTime(), seedMetadata.getAdjustedSeedDataEndTime());
    ...
    var lastGeneratedIntervalId = shiftBackward(seedMetadata, simulationStartTime);

    var initializationSeedTimeRef = new AtomicReference<>(simulationStartTime);

    lastGeneratedIntervalId =
        shiftForward(
            seedMetadata, simulationStartTime, lastGeneratedIntervalId, initializationSeedTimeRef);
    ...
    initializationSeedTimeOffset =
        Duration.between(
            seedMetadata.getAdjustedSeedDataStartTime(), initializationSeedTimeRef.get());
   ...
   * @param initializationSeedTimeOffset How far to go past the initialize time to find the start of
   *     seed data.
   * @param seedData Set of seed intervals.
   * @param nowSupplier A supplier that indicates "now". Normally, would return Instant.now, but can
   *     be made to return whatever is needed for testing.
   * @param intervalIdStart The interval id after which new interval IDs will be created.
   * @param applyCorrections Whether to perform self-correcting. This is set to false for unit
   *     testing
  ...
  static Flux<IntervalDao> getSimulationFlux(
    ...
    var offset =
        Duration.ofMillis(
            initializationSeedTimeOffset
                    .plus(Duration.between(initializationTime, idealStartTime))
                    .toMillis()
                % seedDataLength.toMillis());
    ...
    logger.info(
        "INTERVAL START: initialID: {}, initializationTime: {}, initializationSeedTimeOffset {},"
            + " seedDataLength: {}, idealStartTime: {}, offset: {}, currentSeedTime: {},"
    ...
    return getAnnotatedSeedDataFlux(rotatedSeedData, seedDataLength)
        .repeat()
        .delayUntil(
            intervalDaoDelayPair -> {
              if (applyCorrections && !veryFirstIterationRef.get()) {
                // Find how far we have deviated from the ideal time.
                var difference = Duration.between(currentIdealTimeRef.get(), nowSupplier.get());

                // May not be good if the deviation is this much, so just log a warning.
                if (difference.abs().compareTo(Duration.ofSeconds(1)) > 0) {
                  logger.warn(
                      "Deviation of > 1 second in interval simulation timer: " + difference);
                }

                var newIdealTime = currentIdealTimeRef.get().plus(intervalDaoDelayPair.delay);
                ...
                return Mono.delay(
                    Duration.between(
                        currentSeedTime, rotatedSeedData.get(0).getLastModificationDate()));
              } else {
        ...
        .map(
            intervalDaoDelayPair -> {
              ...
              return createSimulatedInterval(
                  ...
                  currentInterval,
                  Duration.between(currentInterval.getLastModificationDate(), now));
            });
  }

  private SeedMetadata computeSeedMetadata(DataSimulatorSpec intervalSimulatorSpec, Instant now) {
    ...
    var adjustedSeedDuration = Duration.between(adjustedSeedDataStartTime, adjustedSeedDataEndTime);
    ...
    Validate.isTrue(
        initializationEndTime.isAfter(simulationStartTime),
    ...
    var numberForwardSeeds = (int) Math.abs(forwardDuration.dividedBy(adjustedSeedDuration));
    ...
    // additional seed added that will overflow, and that is ok
    var numberBackwardSeeds = (int) Math.abs(backwardsDuration.dividedBy(adjustedSeedDuration));
    ...
    if (backwardsDuration.isNegative()) {
      ...
      logger.info(
          "The configured OperationalTimePeriod {} is less than the duration between  the"
              + " simStartTime, {} and now, {}.  No data will be created before the simStartTime",
          operationalDuration,
          simulationStartTime,
          initializationEndTime);
    }
                ...
                intervalDao ->
                    new IntervalDao.Builder()
                        ...
                        .lastModificationDate(
                            intervalDao.getLastModificationDate().plus(seedDataLength))
                        .build())
            .collect(Collectors.toList()));

    return rotatedSeedData.stream()
        .sorted(
            Comparator.comparing(IntervalDao::getLastModificationDate, Comparator.naturalOrder()))
        .collect(Collectors.toList());
  }
   ...
   *
   * <p>"Annotated" means we are attaching delay information for use in self-correction.
   *
  ...
  private static Flux<IntervalDaoDelayPair> getAnnotatedSeedDataFlux(
      List<IntervalDao> sortedSeedData, Duration seedDataLength) {

    var daoDelayPairs = new ArrayDeque<IntervalDaoDelayPair>();

    IntStream.range(1, sortedSeedData.size() + 1)
        .forEach(
            i -> {
              var myDelay =
                  Duration.between(
              ...
              var pair =
                  new IntervalDaoDelayPair(sortedSeedData.get(i % sortedSeedData.size()), myDelay);
  ...
  private static IntervalDao createSimulatedInterval(
      long newId, IntervalDao intervalDao, Duration timeshift) {
    ...
    IntStream.range(0, seedData.size())
        .forEach(
            i -> {
              ...
              // This is due to the passIndex starting at 1, but needing to be shifted for 0
              // indexing
              // scaled by the number of intervals in the seed and then added to the current index
              // in the
              // current seed
              int currentIndex = ((passIndex - 1) * seedData.size()) + i + 1;
              long timeShift = initialTimeShift - ((passIndex - 1) * seedDuration.getSeconds());

              IntervalDao newDao =
                  buildTimeShiftedInterval(seedData.get(i), timeShift, currentIndex);

              // Mark this interval as done as this data is before the sim time and considered
              // "done"
   ...
   */
  private List<IntervalDao> forwardPassShift(
      long initialTimeShift, int startingIndex, int passIndex, Duration seedDuration) {
    ...
    //
    // Mark them as done if they are automatic intervals, otherwise set them to pending indicating
    // they
    // are ready for an analyst to review them
    //
    // If the shifted interval is after now, remove it from the list of returned values, otherwise
    // add
  ...
  void initializeSeedData(Instant seedDataStartTime, Instant seedDataEndTime) {
    ...
    seedData =
        intervalDatabaseConnector.findIntervalsByTimeRange(seedDataStartTime, seedDataEndTime);

    logger.info(
        "Using {} to {} as the time range for seed data.", seedDataStartTime, seedDataEndTime);
