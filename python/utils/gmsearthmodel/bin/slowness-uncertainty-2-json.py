...
from earth_model_utils import (parse_float_list, parse_int_list, print_error,
                               stdin_exists)
...
def main() -> None:
    ...
    if value_one != 1:
        print_error(
            'Unexpected number of sets of corrections.  Expected 1, saw '
            f'{value_one}.'
        )
...
def get_args() -> dict[str, str]:
    ...
    description = """
description:
  The slowness-uncertainty-2-json command-line program is used to convert ASCII
  whitespace delimited slowness uncertainty tables to JSON format.

  A single ASCII slowness uncertainty table for one model and one phase only is
  read from standard input.  The resulting JSON table is written to standard
  output.
"""
    parser = argparse.ArgumentParser(
        prog='slowness-uncertainty-2-json',
    ...
    if not stdin_exists():
        print_error(
            "One ASCII whitespace slowness uncertainty table must be provided "
            "in standard input."
        )
        sys.exit(1)
