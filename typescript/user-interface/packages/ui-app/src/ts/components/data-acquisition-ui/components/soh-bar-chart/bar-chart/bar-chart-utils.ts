...
export const useSortDropdown = (
  ...
  const useStateMissing = React.useState(SOHMissingOptions.CHANNEL_FIRST);
  const useStateLag = React.useState(SOHLagOptions.CHANNEL_FIRST);
  const useStateTimeliness = React.useState(SOHTimelinessOptions.CHANNEL_FIRST);
  switch (type) {
    case SohTypes.SohMonitorType.MISSING: {
      const [sortOrderMissing, setSortOrderMissing] = useStateMissing;
    ...
    case SohTypes.SohMonitorType.LAG: {
      ...
      const [sortOrderLag, setSortOrderLag] = useStateLag;
    ...
    case SohTypes.SohMonitorType.TIMELINESS: {
      // TODO HANDLE TIMELINESS - try to avoid these specific functions for lag, missing, etc...
      const [sortOrderTimeliness, setSortOrderTimeliness] = useStateTimeliness;
