...
public class UserPreferencesDao {
  ...
  public UserPreferences toCoi() {
    final List<WorkspaceLayout> workspaceLayoutCois =
        workspaceLayouts.stream().map(WorkspaceLayoutDao::toCoi).collect(Collectors.toList());
  ...
  public int hashCode() {
    return Objects.hash(
        userId, defaultAnalystLayoutName, defaultSohLayoutName, currentTheme, workspaceLayouts);
  }
}
