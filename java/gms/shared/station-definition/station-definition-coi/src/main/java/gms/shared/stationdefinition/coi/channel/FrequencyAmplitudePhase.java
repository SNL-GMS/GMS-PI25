...
public abstract class FrequencyAmplitudePhase implements Comparable<FrequencyAmplitudePhase> {
  ...
  public abstract static class Data {
    ...
    public interface Builder {
      ...
      FrequencyAmplitudePhase.Data.Builder setAmplitudeResponseStdDev(
          double[] amplitudeResponseStdDev);
      ...
      default FrequencyAmplitudePhase.Data build() {
        final List<Optional<?>> allFields =
            List.of(
                getFrequencies(),
                getAmplitudeResponseUnits(),
                getAmplitudeResponse(),
                getAmplitudeResponseStdDev(),
                getPhaseResponseUnits(),
                getPhaseResponse(),
     ...
     * return that AmplitudePhaseResponse object. SME guidance for this implementation: For
     * frequency fx, find the nearest lower and higher frequency (f1 and f2), then the interpolated
     * amplitude is ax = a1 + ( (fx - f1) * (a2 - a1) / (f2 - f1) ). Same for phase. You can throw
     * an OutOfRangeException if fx less than f1 or fx greater than f2.
     *
    ...
    public AmplitudePhaseResponse getResponseAtFrequency(Double freq) {
      ...
      if (this.getAmplitudeResponseStdDev()[0]
      ...
      DoubleValue iamp =
          DoubleValue.from(interpAmplitude, Optional.of(stdDevA), this.getAmplitudeResponseUnits());
      DoubleValue iph =
          DoubleValue.from(interpPhase, Optional.of(stdDevP), this.getPhaseResponseUnits());
