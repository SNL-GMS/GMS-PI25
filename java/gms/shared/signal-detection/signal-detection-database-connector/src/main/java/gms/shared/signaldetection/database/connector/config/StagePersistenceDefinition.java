...
public abstract class StagePersistenceDefinition {
  ...
  public ImmutableMap<WorkflowDefinitionId, String> getPreviousDatabaseAccountsByStageMap() {
    return getDatabaseAccountsByStage().stream()
        .filter(StageDatabaseAccountPair::getHasPreviousStage)
        .collect(
            ImmutableMap.toImmutableMap(
                StageDatabaseAccountPair::getWorkflowDefinitionId,
                StageDatabaseAccountPair::getDatabaseAccount));
  ...
  public static StagePersistenceDefinition create(
      @JsonProperty("databaseAccountsByStage")
          ImmutableList<StageDatabaseAccountPair> databaseAccountsByStage) {
    Preconditions.checkState(
        !databaseAccountsByStage.isEmpty(), "Database Accounts per stage have to be set");
