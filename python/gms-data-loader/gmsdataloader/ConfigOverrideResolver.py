...
class ConfigOverrideResolver:
    ...
    def __init__(
        self,
        default_config_root: str,
        override_config_root: str | None = None
    ) -> None:
        """
        default_config_root: the path to the default configuration root
            directory
        override_config_root: if available, the path to the override
    ...
    def path(self, relative_file_path: str) -> str:
        """
        Resolve the full path to a configuration file based on a relative file
            path.

        This will use the override path if available, otherwise it will return
        the default path. If the file is not available, an exception will be
        thrown.
