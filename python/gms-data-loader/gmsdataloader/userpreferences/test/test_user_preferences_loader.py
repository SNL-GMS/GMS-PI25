...
DEFAULT_LOADER_CONFIG = UserPreferencesLoaderConfig(
    'test-ref-loader.org',
...
class TestUserPreferencesLoader(unittest.TestCase):
    ...
    def test_load_dummy_data(self):
        with patch(POST_REQUEST) as mocked_post:
            ...
            mocked_post.assert_has_calls([
                call(
                    DEFAULT_LOADER_CONFIG.user_prefs_obj_urls
    ...
    def test_load_data_post_failure(self):
        ...
        with patch(
            POST_REQUEST,
            new=MagicMock(
                side_effect=mocked_post_user_preferences_respond_500
            )
...
def mocked_post_user_preferences_respond_500(*args, **kwargs) -> MockResponse:
    url = DEFAULT_LOADER_CONFIG.user_prefs_obj_urls.user_preferences_url
    if url in args[0]:
