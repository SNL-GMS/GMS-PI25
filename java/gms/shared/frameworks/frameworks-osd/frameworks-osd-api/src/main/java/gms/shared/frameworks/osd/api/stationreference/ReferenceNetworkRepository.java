...
/** Interface for the endpoint that returns and stores {*link ReferenceNetwork}s */
public interface ReferenceNetworkRepository {

  /**
   * Retrieve all networks.
   *
   * @param networkIds Collection of Entity Ids representing networks to retrieve. If empty, it will
   *     retrieve all networks.
   * @return all networks
   */
  ...
  List<ReferenceNetwork> retrieveNetworks(
      @RequestBody(
              description =
                  "collection of networkIds representing the reference networks to retrieve")
          Collection<UUID> networkIds);

  /**
   * Retrieve all networks for the given list of the name.
   *
   * @param names List of names for all network needs to retrieve. Returns an empty list if nothing
   *     is provided.
  ...
  void storeReferenceNetwork(
      @RequestBody(description = "list of networks to store") Collection<ReferenceNetwork> network);

  /**
   * Retrieves network memberships with the given network entity id.
   *
   * @param networkIds the collection of {@link UUID}s identifying the {@link ReferenceNetwork}s to
   *     retrieve
  ...
  @Operation(
      summary =
          "retrieve a collection of network memberships for the given set of input of "
              + "network ids")
  Map<UUID, List<ReferenceNetworkMembership>> retrieveNetworkMembershipsByNetworkId(
      @RequestBody(
              description =
                  "collection of networkIds that we want the list of network memberships for")
          Collection<UUID> networkIds);

  /**
   * Retrieves network memberships with the given station entity id
   *
   * @param referenceStationIds the {@link ReferenceStation} id of interest
   * @return the memberships
   */
  ...
  Map<UUID, List<ReferenceNetworkMembership>> retrieveNetworkMembershipsByStationId(
      @RequestBody(
              description =
                  "collection of station names that we want the list of network memberships for")
          Collection<UUID> referenceStationIds);

  /**
   * Retrieves network memberships with the given network entity id and station entity id.
   *
   * @param request the {@link NetworkMembershipRequest} containing the IDs of interest
   * @return the memberships
   */
  ...
  @Operation(
      summary =
          "retrieve a collection of network memberships for the given network ID and "
              + "station names")
  List<ReferenceNetworkMembership> retrieveNetworkMembershipsByNetworkAndStationId(
      @RequestBody(
              description =
                  "request containing the given networkId and stationName for which "
