...
if [ "$GMS_UI_MODE" = "ian" ]; then
  cat /etc/nginx/nginx-ian.nginx | envsubst '$URL_PATH $GMS_UI_MODE $NGINX_WORKER_PROCESSES $NODE_ENV' > /etc/nginx/nginx.conf
else
  cat /etc/nginx/nginx-soh-all.nginx | envsubst '$URL_PATH $GMS_UI_MODE $NGINX_WORKER_PROCESSES $NODE_ENV' > /etc/nginx/nginx.conf
fi
