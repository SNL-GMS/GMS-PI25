...
class ReferenceStationTest {
  ...
  private static final UUID stationId =
      UUID.nameUUIDFromBytes(name.getBytes(StandardCharsets.UTF_16LE));
  private static final InformationSource source =
      InformationSource.from("Internet", Instant.now(), "none");
  ...
  private static final UUID versionId =
  ...
  private static ReferenceAlias alias =
      ReferenceAlias.create(aliasName, StatusType.INACTIVE, "no comment", actualTime, systemTime);
  private static List<ReferenceAlias> aliases = List.of(alias);

  @Test
  void testSerialization() throws Exception {
    TestUtilities.testSerialization(
        StationReferenceTestFixtures.REFERENCE_STATION, ReferenceStation.class);
  }

  @Test
  void testReferenceStationBuilder() {
    ReferenceStation sta =
