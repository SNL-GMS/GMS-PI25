...
public interface EventAccessor extends EventRepository {
   ...
   */
  EventsWithDetectionsAndSegments findEventsWithDetectionsAndSegmentsByTime(
   ...
   *
   * @param eventIds A list of {@link UUID}s corresponding to the sought after {@link
   *     EventStatusInfo}s
   * @param stageId The {@link WorkflowDefinitionId} representing the stageId
   * @return {@link EventStatusInfoByStageIdAndEventIdsResponse} which maps eventIds to
   *     EventInfoStatus objects for the given stageId
   */
  EventStatusInfoByStageIdAndEventIdsResponse findEventStatusInfoByStageIdAndEventIds(
      WorkflowDefinitionId stageId, List<UUID> eventIds);

  /**
   * Inserts or updates the {@link EventStatusInfo} for a given {@link Event} inside the
   * EventAccessor EventStatusInfoCache
   *
   * @param stageId The {@link WorkflowDefinitionId} representing the stageId
