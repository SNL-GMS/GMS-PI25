...
from earth_model_utils import (FILE_ENDED_PREMATURELY,
                               expand_units_abbreviation, get_line,
                               parse_float_list, parse_int_list, print_error,
                               print_warning, stdin_exists)
...
def get_args() -> dict[str, str]:
    """
    This function returns a dictionary of command line arguments.  It also
    handles any errors in syntax and help requests.
    :return: dictionary of command line arguments
    """
    description = """
description:
  The travel-time-2-json command-line program is used to convert ASCII
  whitespace delimited travel time tables to JSON format.

  A single ASCII travel time table for one model and one phase only is read
  from standard input.  The resulting JSON table is written to standard output.
"""
    parser = argparse.ArgumentParser(
        prog='travel-time-2-json',
    ...
    if not stdin_exists():
        print_error(
            "One ASCII whitespace delimited travel time table must be "
            "provided in standard input."
        )
        sys.exit(1)
    return args
...
def parse_sample_count(expected_units: str) -> int:
    """
    The next line is expected to start with an integer number of samples and
    end with units in parentheses. This function parses the number of samples
    and units, returns the number of samples, and prints a warning if the units
    differs from the expected value.  If units are not given, expected_units is
    ignored and no warning is given.
...
def seconds_to_duration(x: float | None) -> str | None:
    """
    Given a floating point number in seconds, return the equivalent ISO 8601
    duration string.
