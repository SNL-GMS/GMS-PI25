import type { LegacyEventTypes, StationTypes } from '@gms/common-model';
 ...
 */
export const setDefaultFilterForStations = (stations: StationTypes.Station[]) => (
  dispatch: AppDispatch,
  getState: () => AppState
): void => {
 ...
 * @param stations the stations for which to set the default filter
 */
export const setDefaultFilterForStationsIfFilterIsNotSet = (stations: StationTypes.Station[]) => (
  dispatch: AppDispatch,
  getState: () => AppState
): void => {
  const channelFilters = selectChannelFilters(getState());
