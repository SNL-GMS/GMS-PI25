  ...
  it('should track pending requests in redux state', () => {
    ...
    const expectedState: AnalystState = {
      ...
      requestTracker: {
        ...
        lastRequestId: '3',
        requests: {
          '1': { url: 'https://example.com/foo' },
          '2': { url: 'https://example.com/bar' },
          '3': { url: 'https://example.com/baz' }
        }
      }
    };
    expect(analystSlice.reducer(startingState, action)).toEqual(expectedState);
  ...
  it('should track completed requests in redux state', () => {
    ...
    const expectedState: AnalystState = {
      ...
      requestTracker: {
        ...
        lastRequestId: '3',
        requests: {
          '1': { url: 'https://example.com/foo', isComplete: true, error: undefined },
          '2': { url: 'https://example.com/bar', isComplete: true, error: undefined },
          '3': { url: 'https://example.com/baz', isComplete: true, error: 'ERROR: Baz' }
        }
      }
    };
    expect(analystSlice.reducer(startingState, action)).toEqual(expectedState);
