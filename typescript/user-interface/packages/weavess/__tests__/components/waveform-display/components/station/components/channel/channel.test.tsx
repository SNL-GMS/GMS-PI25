...
describe('Weavess Channel', () => {
  ...
  it('renders the THREE scene when given reasonable DOM rect', () => {
    ...
    const mockSetScissor = jest.fn();
    const mockRender = jest.fn();

    const mockRenderer = {
      setViewport: mockSetViewport,
      setScissor: mockSetScissor,
      render: mockRender
    };
    channelInstance.renderScene((mockRenderer as unknown) as THREE.WebGLRenderer, domRect);
  ...
  it('does not render the THREE scene if DOM rect is out of bounds', () => {
    ...
    channelInstance.renderScene((mockRenderer as unknown) as THREE.WebGLRenderer, mockDomRect);
  ...
  it('create an empty Channel, mount it and call componentDidUpdate', () => {
    ...
    emptyChannelInstance.renderScene((mockRenderer as unknown) as THREE.WebGLRenderer, domRect);
    expect(mockRenderer.setViewport).toHaveBeenCalled();
    expect(mockRenderer.setScissor).toHaveBeenCalled();
    expect(mockRenderer.render).toHaveBeenCalled();
  });
...
describe('Weavess Channel private method', () => {
  ...
  describe('test keyboard events', () => {
    it('call onKeyDown amplitude scaling', () => {
      expect(() =>
        instance.onKeyDown(buildHTMLDivKeyboardEvent(buildKeyboardEvent('KeyY', 'y', true)))
      ).not.toThrow();

      // Set the amplitude scale keyDown followed by the keyup
      expect(() =>
        instance.onKeyDown(buildHTMLDivKeyboardEvent(buildKeyboardEvent('KeyY', 'Y', false)))
      ).not.toThrow();
      ...
      const eventInit: KeyboardEventInit = {
        code: initialConfiguration.hotKeys.scaleWaveformAmplitude.combos[0],
        repeat: false,
        shiftKey: false
      };
    ...
    it('call onKeyDown create mask', () => {
      ...
      expect(() =>
        instance.onKeyDown(buildHTMLDivKeyboardEvent(buildKeyboardEvent('KeyM', 'M', false)))
      ).not.toThrow();
      const eventInit: KeyboardEventInit = {
        code: initialConfiguration.hotKeys.createQcSegments.combos[0],
        repeat: false,
        shiftKey: false
      };
      const keyboardEvent = new KeyboardEvent('keyup', eventInit);
    ...
    it('call onKeyDown', () => {
      const anyKey = buildHTMLDivKeyboardEvent(buildKeyboardEvent('anyKey', 'a', false));
      // Call mask create followed by keyup
...
describe('Weavess Channel with no waveform', () => {
  ...
  it('adds maskTarget class when channel is selected', () => {
    const propsWithMaskTarget = {
      ...
      selections: { ...props.selections, channels: ['channel'] }
    };
