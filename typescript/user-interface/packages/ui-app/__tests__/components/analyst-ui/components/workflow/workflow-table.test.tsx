...
describe('Workflow Table', () => {
  ...
  it('matches snapshot', () => {
    ...
    const expandedRowStatusRecord: Record<string, RowState> = cloneDeep(
      component.state('expandedRowStatusRecord')
    );
    const value: RowState = expandedRowStatusRecord[WorkflowDataTypes.interactiveStage.name];
    expandedRowStatusRecord[WorkflowDataTypes.interactiveStage.name] = {
      ...value,
      isExpanded: true
    };
    component.setState({
