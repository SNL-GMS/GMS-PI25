...
class DziewonskiGilbertLookupTableTest {
  ...
  @Mock DziewonskiGilbertLookupTableConfiguration config;
  @InjectMocks DziewonskiGilbertLookupTable dziewonskiGilbertLookupTable;

  DziewonskiGilbertLookupTableView lookupTableView;

  @BeforeEach
  void init() {
    var minIoBucketName = "MyAmazingBucket";
    var dataDescriptor = "MyAmazingDescriptor";
    ...
    when(config.getCurrentDiezwonskiGilbertLookupTableDefinition()).thenReturn(definition);
  ...
  void testGetUnits() {
    assertEquals(
        Triple.of(Units.SECONDS, Units.SECONDS, Units.SECONDS),
        dziewonskiGilbertLookupTable.getUnits());
  }
  ...
  void testGetDistancesDegForData() {
    ...
    assertEquals(
        expectedDistances, dziewonskiGilbertLookupTable.getDistancesDegForData(PhaseType.P));
  }
