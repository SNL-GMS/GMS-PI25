...
import { sohStatus } from '../../__data__/data-acquisition/soh-status-data';
import { useQueryStateResult } from '../../__data__/query/query-state-result-data';
...
describe('soh-status-buffer tests', () => {
  it('check initial state with no data', () => {
    ...
    expect(() => checkToSendSohStatusChanges()).not.toThrow();
  ...
  it('check state with data added', () => {
    ...
    expect(() => addSohForStation(undefined)).not.toThrow();
    expect(() => addSohForStation(sohStatus.stationAndStationGroupSoh.stationSoh[0])).not.toThrow();
    // ignores duplicates
    expect(() => addSohForStation(sohStatus.stationAndStationGroupSoh.stationSoh[0])).not.toThrow();
  ...
  it('initializeSohStatusBuffering', () => {
    ...
    expect(() => initializeSohStatusBuffering(updateSohMessageFn)).not.toThrow();
  });

  it('consumeUiStationSohKafkaMessages', () => {
    expect(addLatestSohMessages).toBeDefined();
    expect(() => addLatestSohMessages([sohStatus.stationAndStationGroupSoh])).not.toThrow();
    ...
    expect(() => addLatestSohMessages([updateResponse])).not.toThrow();
  });
});
