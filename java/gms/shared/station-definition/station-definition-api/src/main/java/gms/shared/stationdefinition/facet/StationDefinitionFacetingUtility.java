...
public class StationDefinitionFacetingUtility {
  ...
  public Response populateFacets(
    ...
    if (facetingDefinition.isPopulated()) {
      if (initial.isPresent()) {
        // TODO (sgk 04/22/2021 - when FAPRepository is implemented, we'll need to check if the
        // faceting definition
        // indicates if it should be populated and whether it is, and populate it accordingly, but
        // this work is out
        // of scope for PI 15
        return initial;
      } else {
        List<Response> populatedResponses =
            stationDefinitionAccessor.findResponsesById(List.of(initial.getId()), effectiveAt);
        return populatedResponses.isEmpty() ? null : populatedResponses.get(0);
      }
    } else {
      return Response.createVersionReference(
          initial.getId(), initial.getEffectiveAt().orElseThrow());
    }
  }
  ...
  private Station populateChannelGroupsAndChannels(
    ...
    if (channelGroupFacetingDefinition.isPresent()
      ...
      expectedChannels =
          channelList.stream().map(Channel::toEntityReference).collect(Collectors.toList());
    }
    ...
    List<Channel> expectedChannelEntities =
        expectedChannels.stream().map(Channel::toEntityReference).collect(Collectors.toList());
