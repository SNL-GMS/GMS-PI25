...
const getNumberString = (num: number): string => (num < 10 ? `0${num}` : `${num}`);

const getSubSecondDigits = (duration: number) => {
  if (duration > 1) return DEFAULT_NUM_DIGITS;
  // eslint-disable-next-line @typescript-eslint/no-magic-numbers
  if (duration > 0.1) return DEFAULT_NUM_DIGITS + 1;
 ...
 * @returns the formatted time string
 */
export const subMillisecondFormatterFactory = (numberOfDigits = 3) => (
  epochSeconds: number
): string => {
  // eslint-disable-next-line @typescript-eslint/no-magic-numbers
  const EXPANDED_PRECISION = 10 ** numberOfDigits;
...
export class XAxis extends React.PureComponent<XAxisProps, XAxisState> {
  ...
  private readonly internalUpdate = (): void => {
    ...
    if (!useDateScale) {
      ...
      // eslint-disable-next-line @typescript-eslint/no-magic-numbers
      const numMicroSecs = Math.max(Math.round(timeInterval * 10 ** subSecondDigits), 1);
