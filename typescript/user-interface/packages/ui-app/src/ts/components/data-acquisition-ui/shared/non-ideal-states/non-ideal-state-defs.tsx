...
/**
 * Non ideal state definitions for Env Drill Down
 */
export const noEnvContributorsNonIdealStateDefinitions: NonIdealStateDefinition<any>[] = [
  {
    condition: (props): boolean => {
      const station = getStationFromProps(props);
      const envContributors = station.statusContributors.filter(
        contributor => SohTypes.isEnvironmentalIssue(contributor.type) && contributor.contributing
      );
      return envContributors.length === 0;
    },
    element: nonIdealStateWithNoSpinner(
      'No Environmental Monitors',
      'Choose a station with environmental monitors. If Environmental Monitors expected for this station, check the configuration.'
    )
  }
];
