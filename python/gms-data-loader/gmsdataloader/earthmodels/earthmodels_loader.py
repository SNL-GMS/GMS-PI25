...
class EarthModelsLoader:
    ...
    def __init__(
        self,
        earthmodels_root: str,
        earth_model_minio_config: EarthModelMinioConfig
    ) -> None:
        """
        Constructor

        Args:
            earthmodels_root: Path where the JSON models are stored;
                this is the root, which contains directories for each model,
                and each of those directories contains model files.
            earth_model_minio_config: EarthModelMinioConfig objects which
        ...
        for minio_prefix in minio_prefix_map:
            for object_descriptor in minio_prefix_map[minio_prefix]:
                self.minio_client.fput_object(
                    self.bucket,
                    f"{minio_prefix}/{object_descriptor['phase']}",
                    object_descriptor['path']
                )

    def get_minio_prefix_map(self) -> dict:
        """
        Creates a map, where the key is "bucketname/model/model-file-name-
        minus-tilde", where "model-file-name-minus-tilde is the name of the
        file with any leading tilde removed; and the value is the full file
        path to load.
        ...
        # look at our base configuration and get anything that
        # wasn't in overrides
        if not os.path.isdir(self.earthmodels_root):
            raise ValueError(
                f"Processing configuration root {self.earthmodels_root} "
                "must be a directory."
            )

        minio_prefix_map = {}
        directories = [
            (f.name, f.path) for f in os.scandir(self.earthmodels_root)
            if f.is_dir()
        ]  # yapf: disable
        for name, path in directories:

            models = [
                (f.name, f.path) for f in os.scandir(path) if f.is_dir()
            ]  # yapf: disable
            for modelname, modelpath in models:

                minio_key_prefix = f"{name}/{modelname}"
                ...
                for phase_file_name in files:

                    minio_prefix_map[minio_key_prefix].append({
                        'phase': (
                            phase_file_name.replace("~", "")
                            .replace(self.json_file_type, "")
                        ),  # yapf: disable
