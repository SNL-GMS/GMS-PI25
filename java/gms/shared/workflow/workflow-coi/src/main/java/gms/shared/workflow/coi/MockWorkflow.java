...
public final class MockWorkflow {
  ...
  public static Workflow get() {
    ...
    return Workflow.from("Mock Workflow", workflowStages);
  }
  ...
  private static InteractiveAnalysisStage getInteractiveAnalysisStage(String stageName) {
    ...
    var eventReviewActivity =
        Activity.from("Event Review", stationGroup, AnalysisMode.EVENT_REVIEW);

    var scanActivity = Activity.from("Scan", stationGroup, AnalysisMode.SCAN);

    var activities = List.of(eventReviewActivity, scanActivity);

    return InteractiveAnalysisStage.from(stageName, Duration.ofHours(1), activities);
  }
  ...
  private static AutomaticProcessingStage getAutoNetworkStage() {
    ...
    var processingSequence = ProcessingSequence.from("Auto Network Seq", autoNetworkSteps);

    return AutomaticProcessingStage.from(
        "Auto Network", Duration.ofHours(1), List.of(processingSequence));
  }
  ...
  private static AutomaticProcessingStage getAutoPostAl1Stage() {
    ...
    var processingSequence = ProcessingSequence.from("Auto Post-AL1 Seq", sequenceSteps);

    return AutomaticProcessingStage.from(
        "Auto Post-AL1", Duration.ofHours(1), List.of(processingSequence));
  }
}
