...
def parse_float_list(
    num_floats: int | None = None,
    expected_depth: float | None = None,
    expected_units: str | None = None
) -> list[float]:
    """
    This function reads the specified number of whitespace delimited floats
    from one of more input lines, and returns the floats as a list.
    If num_floats is None, then read all floats in the next line on stdin.  One
    line only.
    If expected_depth and expected_units are given (none or both, can not be
    given just one), then a commented line is expected before the list of
    floats from which the depth and units may be parsed.  A warning is issued
    if the read depth and/or units differ from what is expected.
    :param num_floats: number of floats to read
    :param expected_depth: depth value expected in the comment preceding the
        floats
    ...
    if logical_xor(expected_depth is not None, expected_units):
        print_error(
            'parse_float_list() expects neither or both optional args, '
    ...
    if expected_depth:
        ...
        if read_units.casefold() != expected_units.casefold():
            print_warning(
                f'Expected units, {expected_units.casefold()}, '
                f'differs from units in input file, {read_units.casefold()}.'
            )

    if num_floats is not None:
        return read_num_floats(num_floats)
    line = get_line()
    if line is None:
        raise EOFError(FILE_ENDED_PREMATURELY)
...
def read_num_floats(num_floats: int) -> list[float]:
    ...
    to find exactly the number given, and raises an exception if fewer are
    found.
...
def parse_int_list(num_ints: int,
                   exception_on_failure: bool = True) -> list[int]:
    """
    ...
    :param exception_on_failure: if True, raises an exception on failure to
    read specified number of ints.  Otherwise, returns empty list on failure.
...
def get_line(
    ...
    """
    Reads next line (without trailing newline) from standard input, skipping
    # delimited comments unless requested, and skipping blank lines unless
    requested.
    :param skip_comments: do not return commented lines unless this parameter
        is False
    :param skip_blank_lines: do not return blank lines unless this parameter
        is False
    :return: Returns next non-comment / non-blank line from standard input
        without trailing newline.
             Returns next line (whether it is a comment or not) if
                 skip_comments is False.
             Returns next line (whether it is blank or not) if skip_blank_lines
                 is False.
    ...
    for line in sys.stdin:
        ...
        if (
            (len(line) == 0 and skip_blank_lines)
            or (len(line) > 0 and line[0] == '#' and skip_comments)
        ):  # yapf: disable
            continue
        return line
...
def print_warning(message: str) -> None:
    ...
    """
    console.log(rf"[bold yellow]\[WARNING] {message}")
...
def print_error(message: str) -> None:
    ...
    :param message: Message string to print
    """
    console.log(rf"[bold red]\[ERROR] {message}")
    sys.exit(1)
