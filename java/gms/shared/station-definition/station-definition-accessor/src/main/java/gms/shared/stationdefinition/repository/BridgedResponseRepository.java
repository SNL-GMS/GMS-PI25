...
public class BridgedResponseRepository implements ResponseRepository {
  ...
  public List<Response> findResponsesBySiteChanKeys(
      List<SiteChanKey> siteChanKeys, Instant effectiveTime, boolean shouldFilter) {
    ...
    // However, this can be called from the ChannelRepository, which wants all responses to set on
    // the channels, and the channels will be filtered by validRange
    if (shouldFilter) {
  ...
  public Response loadResponseFromWfdisc(long wfdiscRecord) {
    ...
    var responses =
        responseAssembler.buildAllForTimeRange(
            startTime, endTime, List.of(wfdisc), sensors, instruments, channelName);
