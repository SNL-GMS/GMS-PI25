...
/**
 * Represents a {@link Method} annotated by {@link jakarta.ws.rs.Path}. Contains attributes for all
...
public abstract class PathMethod {
  ...
  /**
   * Obtains the full relative path, (including base path but excluding scheme, host, and port), for
   * this object
   *
  ...
  public static PathMethod from(
      String relativePath, Method method, ContentType inputFormat, ContentType outputFormat) {
    return new AutoValue_PathMethod(
        prependSlashIfNotPresent(relativePath), method, inputFormat, outputFormat);
  }
