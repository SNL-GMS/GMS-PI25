...
class ReferenceNetworkMembershipTest {
  ...
  void testSerialization() throws Exception {
    TestUtilities.testSerialization(
        StationReferenceTestFixtures.REFERENCE_NETWORK_MEMBERSHIP,
  ...
  void testReferenceNetworkMembershipCreate() {
    ReferenceNetworkMembership m =
        ReferenceNetworkMembership.create(
            comment, actualTime, systemTime, networkId, stationId, status);

    final UUID expectedId =
        UUID.nameUUIDFromBytes(
            (m.getNetworkId().toString()
                    + m.getStationId()
                    + m.getStatus()
  ...
  void testReferenceNetworkMembFrom() {
    ReferenceNetworkMembership alias =
        ReferenceNetworkMembership.from(
            id, comment, actualTime, systemTime, networkId, stationId, status);
