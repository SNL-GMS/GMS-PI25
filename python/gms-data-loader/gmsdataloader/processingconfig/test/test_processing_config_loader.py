...
import requests
from gmsdataloader.processingconfig import (
    Configuration,
    ProcessingConfigLoader
)

# This does not need to be configurable, since it is just for this test.
URL = "http://127.0.0.1:8080"  # NOSONAR

WORKING_DIR = os.path.dirname(__file__)

DEFAULT_PROC_CFG_LOADER = ProcessingConfigLoader(
    url=URL,
    processing_config_root=f'{WORKING_DIR}/resources/json'
)
...
class TestProcessingConfigLoader(unittest.TestCase):
    ...
    def test_bad_processing_cfg_root(self):
        proc_cfg_loader = ProcessingConfigLoader(
            url=URL,
            processing_config_root="not a directory"
        )
    ...
    def test_get_config_files_single_config_in_one_file(self):
        ...
        self.assertEqual(
            first_value[0],
            {
                'name': self.config_json,
                'path':
                f'{WORKING_DIR}/resources/json/test_config/config.json',
                'override': False
            }
        )
    ...
    def test_create_configurations_single_config_in_one_file(self):
        ...
        self.verify_config_option(
            test_config.configurationOptions[0],
            'test_constraint',
            1
        )
    ...
    def test_get_config_files_multiple_configs_in_one_file(self):
        DEFAULT_PROC_CFG_LOADER = ProcessingConfigLoader(
            url=URL,
            processing_config_root=f'{WORKING_DIR}/resources/'
            'multiple_configurations_in_one_file'
        )
        config_dict = DEFAULT_PROC_CFG_LOADER.get_config_files()
        first_key = next(iter(config_dict.keys()))
        first_value = next(iter(config_dict.values()))
        ...
        self.assertEqual(
            first_value[0],
            {
                ...
                self.config_json,
                'path':
                f'{WORKING_DIR}/resources/multiple_configurations_in_one_file/'
                f'multi_config/config.json',
                'override':
                False
            }
        )
    ...
    def test_create_configurations_multiple_configs_in_one_file(self):
        ...
        self.verify_config_option(
            multi_config.configurationOptions[0],
            self.constraint1_1,
            1
        )
        self.verify_config_option(
            multi_config.configurationOptions[1],
            self.constraint1_2,
            2
        )
        self.verify_config_option(
            multi_config.configurationOptions[2],
            self.constraint2_1,
            1
        )
    ...
    def test_get_config_files_mixed_configs_in_multiple_files(self):
        DEFAULT_PROC_CFG_LOADER = ProcessingConfigLoader(
            url=URL,
            processing_config_root=f'{WORKING_DIR}/resources/'
            'mixed_configurations_in_multiple_files'
        )
        ...
        self.assertTrue({
            ...
            self.config_json,
            'path':
            f'{WORKING_DIR}/resources/mixed_configurations_in_multiple_files/'
            f'multi_config/config.json',
            'override':
            False
        } in config_dict[key])
    ...
    def test_create_configurations_mixed_configs_in_multiple_files(self):
        ...
        sorted_config_options = sorted(
            test_config.configurationOptions,
            key=lambda option: option["name"]
        )
        self.verify_config_option(
            sorted_config_options[0],
            self.constraint1_1,
            1
        )
        self.verify_config_option(
            sorted_config_options[1],
            self.constraint1_2,
            2
        )
        self.verify_config_option(
            sorted_config_options[2],
            self.constraint2_1,
            1
        )
        self.verify_config_option(
            sorted_config_options[3],
            'test_constraint3.1',
            1
        )
        self.verify_config_option(
            sorted_config_options[4],
            'test_constraint3.2',
            2
        )
        self.verify_config_option(
            sorted_config_options[5],
            'test_constraint4.1',
            1
        )
    ...
    def test_get_config_files_mixed_configs_in_one_direcotry(self):
        DEFAULT_PROC_CFG_LOADER = ProcessingConfigLoader(
            url=URL,
            processing_config_root=f'{WORKING_DIR}/resources/'
            'mixed_configurations_in_same_direcotry'
        )
        ...
        self.assertTrue({
            ...
            self.config_json,
            'path':
            f'{WORKING_DIR}/resources/mixed_configurations_in_same_direcotry/'
            f'multi_config/config.json',
            'override':
            False
        } in config_dict[key])
    ...
    def test_post_cfgs(self):
        ...
        # this time is read in from the test/resources/json/test_config/
        # config.json file
        ...
        with patch('requests.Session.post') as mocked_post:
            ...
            mocked_post.assert_has_calls([
                call(
                    url=f'{URL}/processing-cfg/put-all',
                    json=json_list,
                    headers={
    ...
    def test_load(self):
        cfg = Configuration("test")
        data = json.loads(
            '{"name": "test", "configuration_options": '
            '[{"name": "test_constraint", "constraints": '
            '[{"constraintType": "DEFAULT"}], "parameters": '
            '{"defaultNetwork": "demo"}}]}'
        )
    ...
    def test_bad_filetype(self):
        proc_cfg_loader = ProcessingConfigLoader(
            url=URL,
            processing_config_root=f'{WORKING_DIR}/resources/bad_filetype'
        )
        ...
        config_dict = {
            'test_config': [{
                ...
                'bad_filetype.txt',
                'path':
                f'{WORKING_DIR}/resources/bad_filetype/test_config/'
                f'bad_filetype.txt',
                'override':
                False
            }]
    ...
    def test_invalid_input(self):
        proc_cfg_loader = ProcessingConfigLoader(
            url=URL,
            processing_config_root=f'{WORKING_DIR}/resources/invalid_input'
        )
        config_dict = {
            'test_config': [{
                'name':
                'invalid_input.json',
                'path':
                f'{WORKING_DIR}/resources/invalid_input/test_config/'
                'invalid_input.json',
                'override':
                False
            }]
