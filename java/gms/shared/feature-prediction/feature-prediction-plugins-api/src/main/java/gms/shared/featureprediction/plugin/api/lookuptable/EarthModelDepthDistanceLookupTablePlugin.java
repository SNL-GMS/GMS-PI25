 ...
 */
public interface EarthModelDepthDistanceLookupTablePlugin<V, U> extends InitializablePlugin {

  /**
   * Returns the Units associated with the data values returned by the getValues and
   * getStandardDeviations operations.
   *
  ...
  /**
   * Returns an ordered immutable array of depths this model uses to define earth model data for the
   * provided PhaseType. The returned collection is sorted by increasing depth. Depth values have
   * units of kilometers. Returns an empty response if the plugin does not define values for the
   * provided PhaseType.
   *
   ...
   */
  ImmutableDoubleArray getDepthsKmForData(PhaseType phaseType);
   ...
   * values have units of kilometers. Returns an empty response if the plugin does not define values
   * for the provided PhaseType.
   *
   ...
   *     defined for requested PhaseType
   */
  ImmutableDoubleArray getDistancesDegForData(PhaseType phaseType);
   ...
   * distance values returned by the getDepthsKmForData and getDistancesDegForData operations, i.e.
   * the value at the first depth index and the first distance index corresponds to the first depth
   * and first distance in the collections returned by getDepthsKmForData and
   * getDistancesDegForData. The collection is indexed first by depth and then by distance, i.e.
   * values[depthIndex][distanceIndex]. This operation returns an empty response if the plugin does
   * not define values for the provided PhaseType.
   *
   ...
   */
  V getValues(PhaseType phaseType);

  /**
   * Returns an ordered immutable array of depths this model uses to define earth model data value
   * standard deviations for the provided PhaseType. The returned collection is sorted by increasing
   * depth. Depth values have units of kilometers. Returns an empty response if the plugin does not
   * define depth values for standard deviation for the provided PhaseType.
   *
   * @param phaseType the PhaseType for the requested depths
   * @return immutable ordered array of depths in kilometers. Returns empty array if depths not
   *     defined for requested PhaseType
   */
  ImmutableDoubleArray getDepthsKmForStandardDeviations(PhaseType phaseType);

  /**
   * Returns an ordered immutable array of distances this model uses to define earth model data
   * value standard deviations for the provided PhaseType. The returned collection is sorted by
   ...
   */
  ImmutableDoubleArray getDistancesDegForStandardDeviations(PhaseType phaseType);
   ...
   * returned by getDepthsKmForStandardDeviations and getDistancesDegForStandardDeviations. The
   * collection is indexed first by depth and then by distance, i.e.
   * standardDeviations[depthIndex][distanceIndex]. This operation returns an empty response if the
   * plugin does not define standard deviations for the provided PhaseType.
   *
   * @param phaseType the PhaseType for the requested standard deviations
   * @return a 2D ordered collection of standard deviations this plugin defines for the provided
   *     PhaseType.
   */
  V getStandardDeviations(PhaseType phaseType);
}
