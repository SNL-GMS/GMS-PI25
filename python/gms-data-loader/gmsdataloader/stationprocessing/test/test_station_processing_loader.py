...
DEFAULT_LOADER_CONFIG = StationProcessingLoaderConfig(
    'test-proc-loader.org',
    f'{WORKING_DIR}/resources/config/test_config.ini'
)
DEFAULT_STA_PROC_LOADER = StationProcessingLoader(
...
class TestStationProcessingLoader(unittest.TestCase):
    ...
    def test_load_dummy_data(self):
        with patch('requests.post') as mocked_post:
            ...
            mocked_post.assert_has_calls([
                call(
                    DEFAULT_LOADER_CONFIG.station_groups_store_url,
                    json=expected_data
                )
            ])
            ...
            mocked_post.assert_has_calls([
                call(
                    DEFAULT_LOADER_CONFIG.station_groups_update_url,
                    json=expected_data
                )
            ])
    ...
    def test_load_file_system_failure(self):
        with patch('requests.post') as mocked_post:
            # When we fail to read data for station groups, no data should
            # be posted
