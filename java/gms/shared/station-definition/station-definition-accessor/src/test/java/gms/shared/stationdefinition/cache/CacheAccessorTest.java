...
class CacheAccessorTest {
  ...
  @Mock ConfigurationConsumerUtility configurationConsumerUtility;

  @Mock RetryPolicy<Void> retryPolicy;

  @Mock CachePopulator cachePopulator;

  private static final Map<String, Object> operationalTimeConfig =
      Map.of(OPERATIONAL_PERIOD_START, PERIOD_START, OPERATIONAL_PERIOD_END, PERIOD_END);

  private static final Map<String, List<String>> stationGroupMap =
      Map.of(CacheAccessor.STATION_GROUP_NAMES, STATION_GROUP_NAMES);
  ...
  void initialize() {
    when(configurationConsumerUtility.resolve(
            CacheAccessor.OPERATIONAL_TIME_PERIOD_CONFIG, Collections.emptyList()))
        .thenReturn(operationalTimeConfig);

    when(configurationConsumerUtility.resolve(
            CacheAccessor.STATION_GROUP_NAMES_CONFIG, List.of(), Map.class))
        .thenReturn(stationGroupMap);

    cacheAccessor = new CacheAccessor(cachePopulator, configurationConsumerUtility, retryPolicy);
  }
  ...
  void testPopulateCache() {
    try (MockedStatic<CachePopulator> utilities = Mockito.mockStatic(CachePopulator.class)) {
      ...
      verify(cachePopulator, times(1))
          .populate(STATION_GROUP_NAMES, PERIOD_START, PERIOD_END, retryPolicy);
