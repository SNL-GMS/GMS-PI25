...
> {
  ...
  private readonly onSelectionWindowClick = (event: React.MouseEvent<HTMLDivElement>) => {
    ...
    const onMouseMove = (mouseMoveEvent: MouseEvent) => {
      ...
      this.selectionWindowMouseMove(mouseMoveEvent, clientX, htmlEle);
    };
    ...
    const onMouseUp = (mouseUpEvent: MouseEvent) => {
      ...
      if (
        ...
        this.props.onUpdateSelectionWindow
      ) {
  ...
  private readonly selectionWindowMouseMoveWhenDragging = (
    htmlEle: HTMLDivElement,
    diffPct: number,
    diffTimeSecs: number
  ) => {
    ...
    if (
      ...
      if (
        leadPositionPercent < (this.leadBorderRef as MoveableMarker).getMinConstraintPercentage() ||
  ...
  /**
   * Mouse move logic for selection window
   *
   * @param mouseMoveEvent
   * @param clientX
   * @param htmlEle
   */
  private readonly selectionWindowMouseMove = (
    ...
    if (this.mouseDown) {
      ...
      const mouseXOffset = clientX - htmlEle.offsetLeft;
      const viewPortWidth = this.props.viewportClientWidth();
      const percentFrac = 100;
      const startXPercent = (clientX - mouseXOffset) / viewPortWidth;
      ...
      if (this.isDragging && htmlEle && htmlEle.style.left && htmlEle.style.right) {
        this.selectionWindowMouseMoveWhenDragging(htmlEle, diffPct, diffTimeSecs);
      }
    }
  };
