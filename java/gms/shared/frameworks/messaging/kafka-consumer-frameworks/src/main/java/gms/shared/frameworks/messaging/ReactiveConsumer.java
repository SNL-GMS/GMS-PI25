...
/**
 * Functional Interface mirroring {@link Consumer}, but expected to produce a Void {@link Mono} that
 * accepts the data when subscribed to
 *
...
public interface ReactiveConsumer<T> {
  ...
  /**
   * Combines this ReactiveConsumer with an input ReactiveConsumer, accepting the same input data,
   * but runs the input acceptance after this acceptance.
   *
  ...
  /**
   * Helper function for providing a ReactiveConsumer that does nothing. Useful when Classes require
   * a ReactiveConsumer as input, but no processing is necessary.
   *
