...
class WaveformDataSimulatorTest {
  ...
  @Mock private WfdiscDatabaseConnector wfdiscDatabaseConnectorInstance;
  ...
  void testLoadDataAndCleanup() {
    ...
    when(beamDatabaseConnector.findBeamsByWfid(any())).thenReturn(beamDaoList);
    when(idMapper.getOrGenerate(any(AnalysisIdTag.class), anyLong(), anyLong()))
        .thenReturn(defaultId);

    waveformDataSimulator.loadData("stageTest", startTime, endTime, copiedDataTimeShift, idMapper);
    verify(wfdiscDatabaseConnectorInstance, times(1)).findWfdiscsByTimeRange(startTime, endTime);
    verify(beamDatabaseConnector, times(1)).findBeamsByWfid(any());
    ...
    when(beamDatabaseConnector.findBeamsByWfid(any())).thenReturn(beamDaoList2);

    waveformDataSimulator.loadData(STAGE_TEST, startTime, endTime, copiedDataTimeShift, idMapper);
    verify(wfdiscDatabaseConnectorInstance, times(2)).findWfdiscsByTimeRange(startTime, endTime);
    verify(beamDatabaseConnector, times(2)).findBeamsByWfid(any());
  ...
  void testPreload() {
    ...
    waveformDataSimulator.preloadData(
        startTime, endTime, simulationStartTime, operationalTimePeriod, calibUpdateFrequency);
