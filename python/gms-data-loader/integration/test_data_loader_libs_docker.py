...
class TestOsdDataLoadDocker(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.compose.start()
        port = cls.compose.get_service_port("frameworks-osd-service", 8080)
        cls.osd_url = f"http://localhost:{port}/osd"
    ...
    def test_post_ref_data_docker(self):
        station_ref_object_paths = StationReferenceObjectPaths(
            ref_networks_path=f'{RESOURCES_DIR}/json/starefMK01/'
            'reference-network.json',
            ref_stations_path=f'{RESOURCES_DIR}/json/starefMK01/'
            'reference-station.json',
            ref_chans_path=f'{RESOURCES_DIR}/json/starefMK01/'
            'reference-channel.json',
            ref_sites_path=f'{RESOURCES_DIR}/json/starefMK01/'
            'reference-site.json',
            ref_sensors_path=f'{RESOURCES_DIR}/json/starefMK01/'
            'reference-sensor.json'
        )
        station_ref_memb_paths = StationReferenceMembershipPaths(
            ref_net_memb_path=(
                f'{RESOURCES_DIR}/json/starefMK01/'
                'reference-network-membership.json'
            ),
            ref_sta_memb_path=(
                f'{RESOURCES_DIR}/json/starefMK01/'
                'reference-station-membership.json'
            ),
            ref_site_memb_path=(
                f'{RESOURCES_DIR}/json/starefMK01/'
                'reference-site-membership.json'
            )
        )

        sta_ref_loader = StationReferenceLoader(
            StationReferenceLoaderConfig(
    ...
    def test_post_proc_data_docker(self):
        sta_proc_loader = StationProcessingLoader(
            StationProcessingLoaderConfig(
                base_url=self.osd_url,
        ...
        sta_proc_loader.load(
            station_groups_path=f'{RESOURCES_DIR}/json/stagroupCd11MKAR/'
            'station_group.json'
        )
    ...
    def test_update_station_groups(self):
        ...
        sta_proc_loader.load_sta_group_updates(
            station_group_definitions_path=f'{RESOURCES_DIR}/json/'
            'stagroupCd11MKAR/station_group_definition.json'
        )
