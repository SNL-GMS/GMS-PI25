...
import type { CellClickedEvent, CellContextMenuEvent, CellEvent } from 'ag-grid-community';
...
> {
  ...
  private readonly selectCells = (
    ...
    let selectedChannelMonitorPairs =
      ...
      (isAlreadySelected && !shouldRemoveIfExisting)
  ...
  private readonly isMonitorTypeColumn = (params: CellEvent): boolean =>
    params.colDef.colId === headerNameMonitorType;
   ...
   * @param params the table event parameters
   */
  private readonly onCellClicked = (params: CellClickedEvent): void => {
   ...
   * @param params the table event parameters
   */
  private readonly onCellContextMenu = (params: CellContextMenuEvent): void => {
    const data = this.getRows(
      this.props.monitorStatusesToDisplay,
      this.props.sohStatusContributors,
      this.props.channelSohs,
    ...
    const mouseEvent: MouseEvent = params.event as MouseEvent;
    const quietAction = this.generateQuietAction(
      newSelection,
      { left: mouseEvent.x, top: mouseEvent.y },
