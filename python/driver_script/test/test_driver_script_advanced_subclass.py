...
def ensure_phase_comes_next(
    ...
    search_text = (
        ...
        if custom else f"inside '{method_name}' function"
    )
...
def test_stage(
    ...
    index = 0
    phases = [("_run_pre_stage_actions",
               custom_pre_stage),
              ("_begin_stage",
               custom_begin_stage),
              (("run_test",
                False) if "test" in stages_to_run else
               ("_skip_stage",
                custom_skip_stage)),
              ("_end_stage",
               custom_end_stage),
...
def test_stage_retry(
    ...
    for _ in range(retry_attempts):
        phases += [("_prepare_to_retry_stage",
                    custom_prepare_to_retry),
                   ("_begin_stage",
                    False),
                   ("run_test",
                    False),
                   ("_end_stage",
                    False)]
    phases += [("_handle_stage_retry_error",
                custom_handle_retry_error),
               ("_run_post_stage_actions",
                False)]
    for method, custom in phases:
        index = ensure_phase_comes_next(
