...
class IaspeiTravelTimeLookupTableTest {
  ...
  @Mock static FileStore mockedFileStore;
  @Mock static IaspeiTravelTimeLookupTableConfiguration mockedConfiguration;

  private static final TravelTimeLookupTableDefinition definition =
      TravelTimeLookupTableDefinition.builder()
          .setFileDescriptor(FileDescriptor.create("fake-bucket-name", "fake-key-name"))
          .build();
  private static final HashMap<FileDescriptor, TravelTimeLookupView> viewMap = new HashMap<>(2);
  ...
  static void initialize() {
    ...
    TravelTimeLookupView viewP =
        TravelTimeLookupView.builder()
            ...
            .setTravelTimes(
                new Duration[][] {
                  {Duration.parse("PT0.0S"), Duration.parse("PT1212.5273S")},
                  {Duration.parse("PT79.6958S"), Duration.parse("PT1132.8315S")}
                })
            .setModelingErrorDepths(new double[] {0.0, 200.0})
            .setModelingErrorDistances(new double[] {0.0, 180.0})
            .setModelingErrors(
                new Duration[][] {
                  {Duration.parse("PT0.1S"), Duration.parse("PT1.8S")},
                  {Duration.parse("PT0.7S"), Duration.parse("PT1.5S")}
                })
            .build();

    TravelTimeLookupView viewS =
        TravelTimeLookupView.builder()
            .setModel(EarthModelType.IASPEI)
            ...
            .setTravelTimes(
                new Duration[][] {
                  {Duration.parse("PT0.0S"), Duration.parse("PT1636.6226S")},
                  {Duration.parse("PT144.6422S"), Duration.parse("PT1491.9803S")}
                })
            .setModelingErrorDepths(new double[] {})
            .setModelingErrorDistances(new double[] {0.0, 180.0})
            .setModelingErrors(
                new Duration[][] {{Duration.parse("PT0.100S")}, {Duration.parse("PT1.300S")}})
            .build();
  ...
  void setup() {
    Mockito.when(
            mockedFileStore.findByKeyPrefix(
                Mockito.any(FileDescriptor.class),
                Mockito.any(TravelTimeLookupViewTransformer.class)))
        .thenReturn(viewMap);
    Mockito.when(mockedConfiguration.getTravelTimeLookupTableDefinition()).thenReturn(definition);
  ...
  void testUninitializedGetters() {
    ...
    Assertions.assertThrows(
        IllegalArgumentException.class, () -> lookupTable.getDepthsKmForData(PhaseType.P));
    Assertions.assertThrows(
        IllegalArgumentException.class, () -> lookupTable.getDistancesDegForData((PhaseType.P)));
    Assertions.assertThrows(
        IllegalArgumentException.class, () -> lookupTable.getValues(PhaseType.P));
    Assertions.assertThrows(
    ...
    Assertions.assertThrows(
        IllegalArgumentException.class, () -> lookupTable.getStandardDeviations(PhaseType.P));
  }
  ...
  void testUnavailablePhaseType() {
    ...
    Assertions.assertThrows(
        IllegalArgumentException.class, () -> lookupTable.getDepthsKmForData(PhaseType.I));
    Assertions.assertThrows(
        IllegalArgumentException.class, () -> lookupTable.getDistancesDegForData((PhaseType.I)));
    Assertions.assertThrows(
        IllegalArgumentException.class, () -> lookupTable.getValues(PhaseType.I));
    Assertions.assertThrows(
        IllegalArgumentException.class,
    ...
    Assertions.assertThrows(
        IllegalArgumentException.class, () -> lookupTable.getStandardDeviations(PhaseType.I));
  }
}
