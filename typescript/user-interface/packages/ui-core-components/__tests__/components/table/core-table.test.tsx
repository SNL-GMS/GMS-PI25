...
// scenario where headerClass is undefined
...
const columnDefs2: ColumnDefinition<RowType, unknown, unknown, unknown, unknown>[] = [...columns2];
describe('Core Table', () => {
  ...
  const tableProps2: CoreTableProps<RowType, any> = {
    defaultColDef: undefined,
    onCellClicked: jest.fn(),
    onCellContextMenu: jest.fn(),
    onRowSelected: jest.fn(),
    columnDefs: columnDefs2 as any,
    rowData: generateTableRows(),
    debug: true
  };
  ...
  it('can handle select', async () => {
    const wrapper2 = Enzyme.mount(<CoreTable {...tableProps2} />);
    ...
    instance.headerRequiresRefresh();
    instance.tableApi.refreshHeader();
    expect(selectedNodes.length).toBeGreaterThan(0);
    await act(() => {
      expect(() => instance.componentDidUpdate()).not.toThrow();
    });
    await act(() => {
      expect(() => instance.shouldComponentUpdate(tableProps2)).not.toThrow();
    });
  });

  it('headerRequiresRefresh', () => {
    ...
    const spy = jest.spyOn(instance, 'headerRequiresRefresh');
    instance.headerRequiresRefresh();
  ...
  it('logDebugMessage', () => {
    const wrapper = Enzyme.mount(<CoreTable {...tableProps} />);
    const instance: CoreTable<any, any> = wrapper.find(CoreTable).instance() as CoreTable<any, any>;
    const spy = jest.spyOn(instance, 'logDebugMessage');
    instance.logDebugMessage(true, columnDefs[0], true, columnDefs[0]);
    expect(spy).toHaveBeenCalled();
  });
  ...
  it('can clear the datasource if the row model type is infinite', () => {
    const wrapper = Enzyme.mount(<CoreTable {...tableProps} />);
    const instance: CoreTable<any, any> = wrapper.find(CoreTable).instance() as CoreTable<any, any>;
    instance.columnApi = instance.getColumnApi();
    instance.tableApi = {
