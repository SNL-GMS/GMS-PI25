...
class ReferenceDigitizerMembershipTest {
  ...
  void testSerialization() throws Exception {
    TestUtilities.testSerialization(
        StationReferenceTestFixtures.REFERENCE_DIGITIZER_MEMBERSHIP,
  ...
  void testReferenceDigitizerMembershipCreate() {
    ReferenceDigitizerMembership m =
        ReferenceDigitizerMembership.create(
            comment, actualTime, systemTime, digitizerId, channelId, status);
    final UUID expectedId =
        UUID.nameUUIDFromBytes(
            (m.getChannelId().toString()
                    + m.getDigitizerId()
                    + m.getStatus()
                    + m.getActualChangeTime())
  ...
  void testReferenceDigitizerMembFrom() {
    ReferenceDigitizerMembership alias =
        ReferenceDigitizerMembership.from(
            id, comment, actualTime, systemTime, digitizerId, channelId, status);
