...
{{- define "gms.common.secrets.copy" }}
  {{- if .Values.copySecrets }}
    {{- range $key, $secretDef := .Values.copySecrets }}
      ...
      {{- end }}
        {{- include "gms.common.secret.copy" (dict "context" $ "namespace" $secretDef.namespace "source_name" $key "dest_name" ($secretDef.destName | default $key) "optional" ($secretDef.optional | default false)) }}
    {{- end }}
  {{- end }}
{{- end }}
