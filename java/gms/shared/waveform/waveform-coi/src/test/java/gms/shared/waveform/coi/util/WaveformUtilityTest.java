...
class WaveformUtilityTest {
  ...
  private static final File MAIN_DIR =
      new File(WaveformUtilityTest.class.getClassLoader().getResource("data").getFile());
  ...
  static {
    ...
    Waveform waveform2HalfSampleRate =
        Waveform.create(startTime.plusSeconds(1).plusMillis(25), 20.0, samples);
    ...
    Waveform waveform2InterpolatedGap1 =
        Waveform.create(waveform1.getEndTime().plusMillis(25), 40.0, samples);
    ...
    waveform2InterpolatedGap2 =
        Waveform.create(waveform1.getEndTime().plusMillis(50), 40.0, samples);
    ...
    waveform2InterpolatedGap4 =
        Waveform.create(waveform1.getEndTime().plusMillis(100), 40.0, samples);
    ...
    double sampleRateMergedGap1 =
        38.0
            / ((double) Duration.between(startTimeMergedGap1, endTimeMergedGap1).toNanos()
                / 1000000000);
    ...
    double sampleRateMergedGap2 =
        37.0
            / ((double) Duration.between(startTimeMergedGap2, endTimeMergedGap2).toNanos()
                / 1000000000);
  ...
  void testInterpolatedWaveformMaxLimitGap() {
    List<Waveform> waveforms =
        WaveformUtility.interpolateWaveformGap(interpolatedGap2Waveforms, .01, 2.0);
  ...
  void testInterpolatedWaveformDifferentSampleRates() {
    List<Waveform> mergedWaveforms =
        WaveformUtility.interpolateWaveformGap(differentSampleRateWaveforms, 2.0, 1.0);
  ...
  void testInterpolateWaveformGap1() {
    List<Waveform> waveforms =
        WaveformUtility.interpolateWaveformGap(interpolatedGap1Waveforms, .01, 4.0);
  ...
  void testInterpolateWaveformGap2() {
    List<Waveform> waveforms =
        WaveformUtility.interpolateWaveformGap(interpolatedGap2Waveforms, .01, 4.0);
    ...
    // third waveform of 'after' is the same as second waveform of 'before'
    // (waveform2InterpolatedGap2)
    ...
    assertEquals(
        waveform2InterpolatedGap2.getSampleRateHz(), waveformInterp1.getSampleRateHz(), 1.0e-9);
    assertEquals(
        0.5 * (waveform2InterpolatedGap2.getFirstSample() + waveform1.getLastSample()),
        waveformInterp1.getSamples()[0],
        1.0e-9);
    ...
    assertEquals(
        waveform3
            .getStartTime()
            .minusNanos((long) (1000000000 / waveformInterp2.getSampleRateHz())),
        waveformInterp2.getEndTime());
    assertEquals(
        waveform2InterpolatedGap2.getSampleRateHz(), waveformInterp2.getSampleRateHz(), 1.0e-9);
    assertEquals(
        0.5 * (waveform2InterpolatedGap2.getLastSample() + waveform3.getFirstSample()),
        waveformInterp2.getSamples()[0],
        1.0e-9);
  }
  ...
  void testInterpolateWaveformGap4() {
    List<Waveform> waveforms =
        WaveformUtility.interpolateWaveformGap(interpolatedGap4Waveforms, .01, 5.0);
    ...
    assertEquals(
        ...
        waveformInterp1.getEndTime());
    assertEquals(
        waveform2InterpolatedGap4.getSampleRateHz(), waveformInterp1.getSampleRateHz(), 1.0e-9);

    double delta =
        (waveform2InterpolatedGap4.getFirstSample() - waveform1.getLastSample())
            / (waveformInterp1.getSampleCount() + 1);
    for (int i = 0; i < waveformInterp1.getSampleCount(); ++i) {
      assertEquals(
          delta * (i + 1) + waveform1.getLastSample(), waveformInterp1.getSamples()[i], 1.0e-9);
    }
    ...
    assertEquals(
        ...
        waveformInterp2.getEndTime());
    assertEquals(
        waveform2InterpolatedGap4.getSampleRateHz(), waveformInterp2.getSampleRateHz(), 1.0e-9);

    delta =
        (waveform3.getFirstSample() - waveform2InterpolatedGap4.getLastSample())
            / (waveformInterp2.getSampleCount() + 1);
    for (int i = 0; i < waveformInterp2.getSampleCount(); ++i) {
      assertEquals(
          delta * (i + 1) + waveform2InterpolatedGap4.getLastSample(),
          waveformInterp2.getSamples()[i],
          1.0e-9);
    }
  }

  /**
   * Tests that interpolated waveforms are inserted when the sample time difference (using a mean
   * sample rate between the adjacent waveforms) between two consecutive waveforms are just smaller
   * than and just larger than 1.5. In this case the first and second waveforms are separated by a
   * 1.48 sample period and the second and third waveforms are separated by 1.52 a 1.52 sample
   * period. Proper evaluation inserts a single new interpolated waveform between the second and
   * third waveforms, but not between the first and second waveforms since it's sample time
   * difference is less than 1.5.
   */
  @Test
  void testInterpolateWaveformGap1point5() {
    List<Waveform> waveforms =
        WaveformUtility.interpolateWaveformGap(interpolatedGap1_5Waveforms, .01, 4.0);
    ...
    double gapTimeWidth =
        (double)
                Duration.between(waveform2InterpolatedGap1_5.getEndTime(), waveform3.getStartTime())
                    .toNanos()
            / 1000000000;
    ...
    assertEquals(
        0.5 * (waveform2InterpolatedGap1_5.getLastSample() + waveform3.getFirstSample()),
        waveformInterp2.getSamples()[0],
        1.0e-9);
  }
  ...
  private static Stream<Arguments> testCombiningWaveforms() {
    return Stream.of(
        ...
        Arguments.of(
            ChannelSegmentTestFixtures.getTestWfdiscListForCombiningMultipleE1(2), 2.0, 1.5, 1),
        Arguments.of(
            ChannelSegmentTestFixtures.getTestWfdiscListForCombiningMultipleE1(4), 1.0, 1.5, 1),
        Arguments.of(
            ChannelSegmentTestFixtures.getTestWfdiscListForCombiningMultipleE1(6), 1.0, 1.5, 1),
        ...
        Arguments.of(
            ChannelSegmentTestFixtures.getTestWfdiscListWithBigDiffSampRate(), 1.0, 1.5, 2),
        Arguments.of(
            ChannelSegmentTestFixtures.getTestWfdiscListWithSmallDiffSampRate(), 1.0, 1.5, 1));
  }

  @ParameterizedTest
  @MethodSource("testCombiningWaveforms")
  void testMergedWaveforms(
      List<WfdiscDao> wfdiscDaos, double sampTol, double sampDiff, int numWaveforms) {
