...
class ReactiveKafkaStorageConsumerTest {
  ...
  @Mock ReactiveStorageRepository<Collection<String>> mockRepository;
  ...
  void testStoreAndAcknowledge() {
    ...
    storageConsumer =
        new ReactiveKafkaStorageConsumer<>(
            ReactorKafkaUtilities.getValues(),
            mockRepository,
            ReactorKafkaUtilities.acknowledgeAll());

    Flux<List<ReceiverRecord<String, String>>> inputFlux =
        Flux.just(List.of(firstRecord, secondRecord), List.of(thirdRecord, fourthRecord));

    StepVerifier.create(storageConsumer.store(inputFlux)).verifyComplete();
  ...
  void testStore() {
    ...
    storageConsumer =
        new ReactiveKafkaStorageConsumer<>(
            ReactorKafkaUtilities.getValues(), mockRepository, ReactiveConsumer.doNothing());

    Flux<List<ReceiverRecord<String, String>>> inputFlux =
        Flux.just(List.of(firstRecord, secondRecord));
  ...
  void testStoreFailsOnIrrecoverableFailure() {
    ...
    willThrow(IllegalStateException.class).given(mockRepository).storeInternal(storeValue);
  ...
  void testEmptyPreprocessDoesNotStoreStillAcknowledges() {
    ...
    storageConsumer =
        new ReactiveKafkaStorageConsumer<>(
            values -> Mono.empty(), mockRepository, ReactorKafkaUtilities.acknowledgeAll());
  ...
  static ReceiverRecord<String, String> record(String value) {
    return new ReceiverRecord<>(
        new ConsumerRecord<>("test", 0, 0, "test", value), Mockito.mock(ReceiverOffset.class));
  }
}
