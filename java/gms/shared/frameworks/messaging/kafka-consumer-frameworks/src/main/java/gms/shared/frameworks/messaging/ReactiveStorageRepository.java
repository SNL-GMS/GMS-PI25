...
/**
 * Interface for defining a Repository-type class that stores reactively. All reactive store calls
 * are managed via {@link Mono#fromRunnable(Runnable)}
 *
 * @param <T>
 */
@FunctionalInterface
public interface ReactiveStorageRepository<T> {
   ...
   * @return Void Mono that will store the input value when subscribed to, and retry the store call
   *     when appropriate
   */
