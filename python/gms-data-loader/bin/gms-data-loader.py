...
def setup_station_reference_parser(parser: argparse.ArgumentParser):
    ...
    # TODO: remove default='dataloaders/stationreference/resources/config/
    #       config.ini')
    parser.add_argument(
        '--ref_networks',
        help='Path to Reference Networks Array (JSON)',
        required=False,
        default='stationdata/reference-network.json'
    )
    parser.add_argument(
        '--ref_stations',
        help='Path to Reference Stations Array (JSON)',
        required=False,
        default='stationdata/reference-station.json'
    )
    parser.add_argument(
        '--ref_sites',
        help='Path to Reference Sites Array (JSON)',
        required=False,
        default='stationdata/reference-site.json'
    )
    parser.add_argument(
        '--ref_chans',
        help='Path to Reference Channels Array (JSON)',
        required=False,
        default='stationdata/reference-channel.json'
    )
    parser.add_argument(
        '--ref_sensors',
        help='Path to Reference Sensors Array (JSON)',
        required=False,
        default='stationdata/reference-sensor.json'
    )
    parser.add_argument(
        '--ref_net_memberships',
        help='Path to Reference Network Memberships Array (JSON)',
        required=False,
        default='stationdata/reference-network-memberships.json'
    )
    parser.add_argument(
        '--ref_sta_memberships',
        help='Path to Reference Station Memberships Array (JSON)',
        required=False,
        default='stationdata/reference-station-memberships.json'
    )
    parser.add_argument(
        '--ref_site_memberships',
        help='Path to Reference Site Memberships Array (JSON)',
        required=False,
        default='stationdata/reference-site-memberships.json'
    )
    parser.set_defaults(action=load_station_reference)


def load_station_reference(args: argparse.Namespace):
    loader = StationReferenceLoader(
        StationReferenceLoaderConfig(args.url,
                                     args.config)
    )

    sta_ref_obj_paths = StationReferenceObjectPaths(
        ref_networks_path=args.ref_networks,
        ref_stations_path=args.ref_stations,
        ref_sites_path=args.ref_sites,
        ref_chans_path=args.ref_chans,
        ref_sensors_path=args.ref_sensors
    )
    sta_ref_memb_paths = StationReferenceMembershipPaths(
        ref_net_memb_path=args.ref_net_memberships,
...
def setup_station_processing_parser(parser: argparse.ArgumentParser):
    ...
    # TODO: remove default='dataloaders/stationprocessing/resources/config/
    #       config.ini')
    parser.add_argument(
        '--station_groups',
        help='Path to Station Groups Array (JSON)',
        required=False,
...
def load_station_processing(args: argparse.Namespace):
    loader = StationProcessingLoader(
        StationProcessingLoaderConfig(args.url,
                                      args.config)
    )
...
def setup_station_group_update_parser(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        '-f',
        '--file',
        help='Path to Station Group Definition Array (JSON)',
        required=True
    )
    parser.add_argument(
        '-c',
...
if __name__ == '__main__':
    ...
    setup_station_reference_parser(
    ...
    setup_station_processing_parser(
        command_parser.add_parser(
            'load-station-processing',
            aliases=['lsp'],
            help='Load Station Processing Data into the OSD'
        )
    )
    setup_user_preferences_parser(
        command_parser.add_parser(
    ...
    setup_station_group_update_parser(
        command_parser.add_parser(
            'update-station-groups',
            aliases=['usg'],
            help='Update Station Groups in the OSD using Station Group '
            'Definitions'
        )
    )
    if len(sys.argv) <= 1:
