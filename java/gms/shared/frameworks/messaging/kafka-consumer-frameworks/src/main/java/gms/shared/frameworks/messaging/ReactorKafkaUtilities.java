...
public class ReactorKafkaUtilities {
  ...
  /**
   * Uses Reactive Kafka to generate a flux of incoming {@link ReceiverRecord}s, batching them via a
   * buffer configured using the kafka polling configuration. The goal is for every inner batch
   * collection to represent a single poll to the kafka broker.
   *
   ...
   */
  public static <T> Flux<List<ReceiverRecord<String, T>>> createBatchRecordFlux(
      SystemConfig systemConfig, Deserializer<T> deserializer) {
    ...
    ReceiverOptions<String, T> receiverOptions =
        kafkaConfiguration
            .getReceiverOptions(deserializer, inputTopic)
            .schedulerSupplier(Schedulers::boundedElastic);

    return KafkaReceiver.create(receiverOptions)
        .receive()
        .bufferTimeout(
            kafkaConfiguration.getMaxPollRecords(),
  ...
  public static RetryBackoffSpec retryForever(Logger logger) {
    return Retry.backoff(Long.MAX_VALUE, Duration.ofMillis(100))
        ...
        .doAfterRetry(
            retry ->
                logger.error(
                    "Error encountered while running kafka consumer, attempting re-subscription...",
                    retry.failure()));
  }

  /**
   * Generates a {@link ReactiveFunction} that extracts and collects all values in a batch of {@link
   * ReceiverRecord}s
   *
   * @param <T> Value type of the receiver record and returned collection of values
   * @return ReactiveFunction that extracts and collects all values in a batch of {@link
   *     ReceiverRecord}s
   */
  public static <T>
      ReactiveFunction<Collection<ReceiverRecord<String, T>>, Collection<T>> getValues() {
  ...
  /**
   * Generates a {@link ReactiveConsumer} that acknowledges all input {@link ReceiverRecord}s from a
   * batch
   *
   * @param <T> Type of value in the record
