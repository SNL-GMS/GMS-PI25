...
/** Accessor interface for retrieving {@link ChannelGroup}s */
public interface ChannelGroupAccessor extends ChannelGroupRepository {

  /**
   * Retrieves all {@link ChannelGroup} objects specified by a list of channel group names,
   * effective time instant and faceting definition. If the list is empty, the server will return an
   * empty list of channel groups.
   *
   ...
   */
  List<ChannelGroup> findChannelGroupsByNameAndTime(
      List<String> channelGroupNames, Instant effectiveTime, FacetingDefinition facetingDefinition);
}
