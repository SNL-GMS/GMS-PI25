...
from python.utils.skipped_tests.skipped_tests.jest import Jest


@pytest.fixture
def skipped_tests() -> Jest:
    test_root = Path(__file__).resolve().parent / "test_root"
    return Jest(test_root)


@pytest.fixture
def sample_file(skipped_tests: Jest) -> Path:
    contents = """
...
def test_init(skipped_tests: Jest) -> None:
