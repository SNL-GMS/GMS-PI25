...
class ReferenceSensorTest {
  ...
  void testSerialization() throws Exception {
    TestUtilities.testSerialization(
        StationReferenceTestFixtures.REFERENCE_SENSOR, ReferenceSensor.class);
  }
  ...
  void testReferenceSensorCreate() {
    ...
    final UUID expectedId =
        UUID.nameUUIDFromBytes(
            (sensor.getChannelName()
                    + sensor.getInstrumentManufacturer()
                    + sensor.getInstrumentModel()
                    + sensor.getSerialNumber()
                    + sensor.getNumberOfComponents()
                    + sensor.getCornerPeriod()
                    + sensor.getLowPassband()
                    + sensor.getHighPassband()
                    + sensor.getActualTime()
                    + sensor.getSystemTime())
    ...
    assertEquals(
        StationReferenceTestFixtures.INSTRUMENT_MANUFACTURER, sensor.getInstrumentManufacturer());
    ...
    assertEquals(
        StationReferenceTestFixtures.CORNER_PERIOD,
        sensor.getCornerPeriod(),
        StationReferenceTestFixtures.PRECISION);
    assertEquals(
        StationReferenceTestFixtures.LOW_PASSBAND,
        sensor.getLowPassband(),
        StationReferenceTestFixtures.PRECISION);
    assertEquals(
        StationReferenceTestFixtures.HIGH_PASSBAND,
        sensor.getHighPassband(),
        StationReferenceTestFixtures.PRECISION);
