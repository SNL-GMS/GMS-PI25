...
class CachePopulatorTest {
  ...
  private static StationDefinitionAccessor stationDefinitionAccessor;
  ...
  @BeforeAll
  static void setup() {
    stationDefinitionAccessor = mock(StationDefinitionAccessor.class);
  ...
  void testCreate() {
    CachePopulator cachePopulator =
        assertDoesNotThrow(() -> new CachePopulator(stationDefinitionAccessor));
    assertNotNull(cachePopulator);
  }

  @ParameterizedTest
  @MethodSource("getPopulateValidationArguments")
  void testPopulateValidation(
    ...
    assertThrows(
        expectedException,
        () -> cachePopulator.populate(stationGroupNames, periodStart, periodEnd, retryPolicy));
  }

  static Stream<Arguments> getPopulateValidationArguments() {
    return Stream.of(
        arguments(NullPointerException.class, null, PERIOD_START, PERIOD_END, retryPolicy),
        arguments(NullPointerException.class, null, PERIOD_START, PERIOD_END, retryPolicy),
        arguments(NullPointerException.class, STATION_GROUP_NAMES, null, PERIOD_END, retryPolicy),
        arguments(NullPointerException.class, STATION_GROUP_NAMES, PERIOD_START, null, retryPolicy),
        arguments(NullPointerException.class, STATION_GROUP_NAMES, PERIOD_START, PERIOD_END, null),
        arguments(
  ...
  @Test
  void testPopulate() {
    when(stationDefinitionAccessor.findStationGroupsByNameAndTimeRange(
            eq(STATION_GROUP_NAMES), any(), any()))
        .thenReturn(List.of(STATION_GROUP));
    List<String> stationNames =
        STATION_GROUP.getStations().stream().map(Station::getName).collect(Collectors.toList());
    ...
    when(stationDefinitionAccessor.findChannelGroupsByNameAndTimeRange(
            eq(channelGroupNames), any(), any()))
        .thenReturn(List.of(CHANNEL_GROUP));
    List<String> channelNames =
        CHANNEL_GROUP.getChannels().stream().map(Channel::getName).collect(Collectors.toList());
  ...
  @Test
  @Disabled(
      "this test takes several minutes to complete.  Re-enable when retry policy is configurable")
  void testPopulateRetry() {
    ...
    verify(stationDefinitionAccessor, times(10))
        .findStationGroupsByNameAndTimeRange(eq(STATION_GROUP_NAMES), any(), any());
    verifyNoMoreInteractions(stationDefinitionAccessor);
  }
}
