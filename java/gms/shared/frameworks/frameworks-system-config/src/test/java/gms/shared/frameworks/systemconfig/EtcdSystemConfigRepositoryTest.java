...
class EtcdSystemConfigRepositoryTest {
  ...
  void testBuilder() {
    ...
    var configRepo =
        EtcdSystemConfigRepository.builder()
  ...
  void testGet() {
    ...
    var response =
        new GetResponse(RangeResponse.newBuilder().addKvs(keyValue).build(), ByteSequence.EMPTY);

    given(mockBuilder.buildClient()).willReturn(mockClient);
    given(mockClient.getKVClient()).willReturn(mockKV);
    given(mockKV.get(ByteSequence.from(inputKey, StandardCharsets.UTF_8)))
