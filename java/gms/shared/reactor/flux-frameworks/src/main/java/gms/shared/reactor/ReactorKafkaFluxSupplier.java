...
/**
 * Extension of {@link FluxSupplier} to incorporate grouping by kafka partitions and extracting of
 * values from {@link ReceiverRecord}s
 *
 * @param <V>
 */
@FunctionalInterface
public interface ReactorKafkaFluxSupplier<V> extends FluxSupplier<ReceiverRecord<String, V>> {
  ...
  default Flux<GroupedFlux<TopicPartition, V>> getPartitionValueFlux() {
    return toGroupedFlux(
        receiverRecord -> receiverRecord.receiverOffset().topicPartition(), ReceiverRecord::value);
  }
}
