...
import { NOT_APPLICABLE, UNKNOWN } from '../types';
...
export const formatSohValue = (value: number): string => {
  if (value === UNKNOWN || Number.isNaN(value) || value === undefined) {
    return 'Unknown';
  }
...
export enum ContributingStatus {
 ...
 */
export const getCellStatus = (status: SohTypes.SohStatusSummary): CellStatus => {
  ...
  throw new Error(`Soh status summary is not valid ${status}`);
};

/**
 * Determines which Contributing Status to return
 *
 * @param isContributing boolean if contributing
 * @returns Contributing Status
 */
export const getContributingStatus = (isContributing: boolean): ContributingStatus => {
  return isContributing ? ContributingStatus.CONTRIBUTING : ContributingStatus.NOT_CONTRIBUTING;
};
...
export const getDataReceivedStatus = (data: CellData | number): DataReceivedStatus => {
  ...
  const isNull = isNullData(data)
    ? DataReceivedStatus.NOT_ENOUGH_DATA
    : DataReceivedStatus.RECEIVED;
  return isNullStatus(data) ? DataReceivedStatus.NOT_RECEIVED : isNull;
};
...
export const setTooltip = (data: CellData): string => {
  const noDataMessageStatus = isNullStatus(data) ? notReceivedTooltipMessage : !data.isContributing;
  const noDataMessage = isNullData(data) ? nullTooltipMessage : '';
  return noDataMessageStatus ? nonContributingTooltipMessage : noDataMessage;
};
