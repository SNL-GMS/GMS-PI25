...
import type { CellStatus, ContributingStatus, DataReceivedStatus } from './utils';
...
export function SohRollupCell(props: SohCellRendererProps) {
  ...
  const lastPosition = // eslint-disable-next-line no-unsafe-optional-chaining
    index && index === props?.columnApi?.getAllDisplayedColumns().length - 1 ? 'last' : index;
  const columnPosition = index && index === 0 ? 'first' : lastPosition;
  return (
    <TableCellRenderer
      data-cell-status={props.cellStatus?.toString()}
      data-cell-contributing-status={props.contributingStatus?.toString()}
