...
class BridgedRepositoryUtilTest {
  ...
  @Mock SensorDatabaseConnector sensorDatabaseConnector;

  @Mock InstrumentDatabaseConnector instrumentDatabaseConnector;
  ...
  void testGetMinMaxFromSiteDaos() {
    ...
    var pairTime =
        BridgedRepositoryUtils.getMinMaxFromSiteDaos(List.of(siteDao1, siteDao2), t1, t2);
  ...
  void testGetSensorAndWfdiscData() {
    ...
    sensor1.getSensorKey().setEndTime(t2);
    var wfdisc = DefaultCoiTestFixtures.getDefaultWfdisc();

    var listOfSiteChanKeys = List.of(siteChan1.getId(), siteChan2.getId());

    when(sensorDatabaseConnector.findSensorsByKeyAndTimeRange(listOfSiteChanKeys, tStart, tEnd))
        .thenReturn(List.of(sensor1));
    when(wfdiscDatabaseConnector.findWfdiscsByNameAndTimeRange(listOfSiteChanKeys, t4, tEnd))
        .thenReturn(List.of(wfdisc));

    var dataHolder =
        new StationDefinitionDataHolder(
            List.of(), List.of(siteChan1, siteChan2), List.of(), List.of(), List.of(), null);

    var dataHolderReturned =
        BridgedRepositoryUtils.getSensorAndWfdiscData(
            dataHolder, sensorDatabaseConnector, wfdiscDatabaseConnector);
  ...
  void testGetInstrumentData() {
    ...
    var dataHolder =
        new StationDefinitionDataHolder(
            List.of(), List.of(), List.of(sensor1), List.of(), List.of(), null);

    var instrument = DefaultCoiTestFixtures.getDefaultInstrumentDao();
    when(instrumentDatabaseConnector.findInstruments(
            List.of(sensor1.getInstrument().getInstrumentId())))
