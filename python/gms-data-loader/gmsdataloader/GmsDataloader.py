...
from gmsdataloader.stationprocessing import (
    StationProcessingLoader,
    StationProcessingLoaderConfig
)
from gmsdataloader.stationreference import (
...
class GmsDataloader:
    ...
    def load_station_reference_data(self, osd_url: str) -> None:
        station_reference_loader = StationReferenceLoader(
            StationReferenceLoaderConfig(osd_url)
        )

        # locate station reference files
        ref_networks_path = self.resolver.path(
            "station-reference/stationdata/reference-network.json"
        )
        ref_stations_path = self.resolver.path(
            "station-reference/stationdata/reference-station.json"
        )
        ref_sites_path = self.resolver.path(
            "station-reference/stationdata/reference-site.json"
        )
        ref_chans_path = self.resolver.path(
            "station-reference/stationdata/reference-channel.json"
        )
        ref_sensors_path = self.resolver.path(
            "station-reference/stationdata/reference-sensor.json"
        )

        station_reference_object_paths = StationReferenceObjectPaths(
            ref_networks_path=ref_networks_path,
        ...
        ref_net_memb_path = self.resolver.path(
            "station-reference/stationdata/reference-network-memberships.json"
        )
        ref_sta_memb_path = self.resolver.path(
            "station-reference/stationdata/reference-station-memberships.json"
        )
        ref_site_memb_path = self.resolver.path(
            "station-reference/stationdata/reference-site-memberships.json"
        )

        station_reference_membership_paths = StationReferenceMembershipPaths(
            ref_net_memb_path=ref_net_memb_path,
            ref_sta_memb_path=ref_sta_memb_path,
            ref_site_memb_path=ref_site_memb_path
        )

        station_reference_loader.load(
            station_reference_object_paths,
            station_reference_membership_paths
        )

    def load_station_processing_data(self, osd_url: str) -> None:
        station_groups_path = self.resolver.path(
            "station-reference/stationdata/processing-station-group.json"
        )
        station_processing_loader = StationProcessingLoader(
            StationProcessingLoaderConfig(osd_url)
        )
    ...
    def update_station_group_definitions(self, osd_url: str) -> None:
        # Note: this file is optional and may not be present.  It is just a
        # no-op if this is not there.
        relative_station_group_definitions_path = (
            "station-reference/definitions/"
            "processing-station-group-definition.json"
        )
        try:
            station_group_definitions_path = self.resolver.path(
                relative_station_group_definitions_path
            )
            station_processing_loader = StationProcessingLoader(
                StationProcessingLoaderConfig(osd_url)
            )
            station_processing_loader.load_sta_group_updates(
                station_group_definitions_path=station_group_definitions_path
            )
        except requests.HTTPError as ex:
            logger.error(
                f"Failed to update station groups based on "
                f"'{relative_station_group_definitions_path}'. {ex}"
            )
        except OSError:
            # Not an error. Note to the log that no action needs to be taken.
            logger.info(
                f"No '{relative_station_group_definitions_path}' file present."
                " No action to take."
            )
