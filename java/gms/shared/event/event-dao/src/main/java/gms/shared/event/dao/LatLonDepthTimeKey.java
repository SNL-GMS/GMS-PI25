  ...
  public static class Builder {
    ...
    public LatLonDepthTimeKey build() {
      ...
      if (!DoubleMath.fuzzyEquals(depth, NA_VALUE, NA_EPSILON)) {
        checkArgument(
            (-100.0 <= depth) && (depth < 1000.0),
            "Depth is " + depth + DaoHelperUtility.createRangeStringDouble(-100, 1000, '(', ']'));
      }

      // NA not allowed
      checkArgument(
          -9_999_999_999.999 < time,
          "Time is " + time + ".  It must be greater than -9,999,999,999.999.");
