...
public class SiteChanDatabaseConnector extends DatabaseConnector {
  ...
  @Autowired
  public SiteChanDatabaseConnector(
                                  ...
                                  k ->
                                      cb.and(
                                          ...
                                          cb.greaterThanOrEqualTo(
                                              fromSiteChan.get(OFF_DATE), startTime),
                                          cb.lessThanOrEqualTo(idPath.get(ON_DATE), endTime)))
                                ...
                                keySublist.stream()
                                    .map(
                                        key ->
                                            builder.and(
                                                builder.equal(
                                                    id.get(STATION_CODE), key.getStationCode()),
                                                builder.equal(
                                                    id.get(CHANNEL_CODE), key.getChannelCode())))
                                    .toArray(Predicate[]::new))),
                            ...
                            partition.stream()
                                .map(
                                    key ->
                                        cb.and(
                                            cb.equal(
                                                idPath.get(STATION_CODE), key.getStationCode()),
                                            cb.equal(
                                                idPath.get(CHANNEL_CODE), key.getChannelCode())))
                                .toArray(Predicate[]::new)),
                        cb.lessThanOrEqualTo(idPath.get(ON_DATE), effectiveAt),
                        cb.greaterThanOrEqualTo(fromSiteChan.get(OFF_DATE), effectiveAt));
