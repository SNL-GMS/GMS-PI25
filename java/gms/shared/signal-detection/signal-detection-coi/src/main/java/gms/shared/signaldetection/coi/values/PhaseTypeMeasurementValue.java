...
@AutoValue
public abstract class PhaseTypeMeasurementValue implements EnumeratedMeasurementValue<PhaseType> {
  ...
  public static PhaseTypeMeasurementValue from(
    ...
    confidence.ifPresent(
        confidenceVal ->
            checkState(
                0.0 <= confidenceVal && confidenceVal <= 1.0,
                "Confidence must be between 0.0 and 1.0 inclusive"));
    return new AutoValue_PhaseTypeMeasurementValue(value, confidence, referenceTime);
  }

  /**
   * Create {@link PhaseTypeMeasurementValue} feature measurement from phase type, confidence and
   * reference time
   *
   ...
   */
  public static PhaseTypeMeasurementValue fromFeatureMeasurement(
      PhaseType value, Optional<Double> confidence, Instant referenceTime) {
    return from(value, confidence, Optional.of(referenceTime));
  }

  /**
   * Create {@link PhaseTypeMeasurementValue} feature measurement from phase type and confidence
   *
   * @param value {@link PhaseType} input
   * @param confidence optional confidence value
   * @return {@link PhaseTypeMeasurementValue} feature measurement value
   */
  public static PhaseTypeMeasurementValue fromFeaturePrediction(
      PhaseType value, Optional<Double> confidence) {
