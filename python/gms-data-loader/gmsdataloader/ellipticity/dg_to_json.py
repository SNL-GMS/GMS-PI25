...
from gmsdataloader.ellipticity.earth_model_file_utils import (
    get_line,
    parse_float_list
)
...
def parse_phase_table(input_file_name: str) -> dict[str, str] | None:
    """
    This function parses standard input for the tau tables for the next phase
    type at standard input.  It reads the phase type, the list of depths, and a
    set of tau0, tau1, tau2 for each distance.  It returns a dictionary with
    the phase, list of depths, list of distances, and three tau tables: tau0,
    tau1, and tau2.  The tau tables are each a 2-D array (i.e., a list of
    lists).  The first index is depth, and the second index is distance. If
    standard input is at end of file, None is returned.
    :return: A dictionary of tau tables for one phase type.
    ...
    return {
        'phase': parse_phase(
            input_file_name.split('_', maxsplit=2)[-1]
        ),  # yapf: disable
        'depths': depths,
        'distances': distances,
        'tau0': tau0,
...
def get_filename(prefix: str, phase: str) -> str:
    """
    Returns the filename for the given phase.  Filenames will be the prefix
    followed by the phase, and ".json".  The exception is for phases that start
    with a lower-case 's' or 'p'.  The names of those files will be the prefix
    followed by a tilda ('~'), followed by the phase, and ".json". This will
    make files distinguishable on case-insensitive operating systems.
...
def replace_str_in_phase(phase: str, replace: str) -> str:
    ...
    while replace in new_phase:
        ...
        if underscore_index != -1:
            new_phase = (
                new_phase[:replace_index]
                + new_phase[replace_index + len(replace):underscore_index]
                + new_phase[underscore_index + 1:]
            )
        else:
            new_phase = (
                new_phase[:replace_index]
                + new_phase[replace_index + len(replace):]
            )
    return new_phase
